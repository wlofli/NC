...
public class CmsApp extends App {
	...
	public String dumpSql() {
		...
		for (DataModel model : modelList) {
			...
			String tbSql = sqlExportService.getCreateTableSql(baseDBRouter
					.getTableName(model.getEnglish_name()));
			String tbName = "es_" + model.getEnglish_name()
					+ "_<userid>_<siteid>";
	...
	protected void cleanCache() {
		super.cleanCache();
		// 清除挂件缓存
		CacheFactory.getCache(DataCatCacheProxy.cacheName).remove(
				DataCatCacheProxy.cacheName + "_" + userid + "_" + siteid);
	}
