...
/**
 * 文章分类缓存代理<br>
 * 首先在分类缓存中以cacheName+"_"+site.getUserid() +"_"+site.getId();
 * 为key存储当站点的分类缓存。以实现站点缓存隔离<br>
 * 在上述缓存中再以 cacheName+"_cat_"+catid;为key存储各分类的所有子类数据。
 * 
 * @author kingapex 2010-7-5上午09:46:44
 */
public class DataCatCacheProxy extends AbstractCacheProxy<Map> implements
	...
	private void put(String key, List<DataCat> list) {
		...
		String mainkey = cacheName + "_" + site.getUserid() + "_"
				+ site.getId();
