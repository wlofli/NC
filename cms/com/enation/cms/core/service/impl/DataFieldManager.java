 ...
 * 模型数据字段管理
 * 
 * @author kingapex 2010-7-2下午03:41:44
 */
public class DataFieldManager extends BaseSupport<DataField> implements
		IDataFieldManager {
	...
	public Integer add(DataField dataField) {
		...
		DataModel datamodel = this.dataModelManager
				.get(dataField.getModel_id());
	...
	public void delete(Integer fieldid) {
		...
		String sql = "alter table "
				+ this.getModelTableName(dataModel.getEnglish_name())
				+ " drop " + dataField.getEnglish_name();
	...
	public void edit(DataField dataField) {
		...
		this.baseDaoSupport.update("data_field", dataField, "field_id="
				+ dataField.getField_id());
		if (!oldDataField.getEnglish_name().equals(dataField.getEnglish_name())) {// 变更了字段名
			DataModel dataModel = this.dataModelManager.get(dataField
					.getModel_id());
			StringBuffer sql = new StringBuffer();
			sql.append("alter table ");
			sql.append(this.getModelTableName(dataModel.getEnglish_name()));
	...
	public List<DataField> list(Integer modelid) {
		return this.baseDaoSupport.queryForList(
				"select * from data_field where model_id=? order by taxis",
				DataField.class, modelid);
	}

	public DataField get(Integer fieldid) {
		String sql = "select * from data_field where field_id=?";
		return this.baseDaoSupport
				.queryForObject(sql, DataField.class, fieldid);
	}
	...
	private String getFieldTypeSql(DataField field) {
		...
		return field.getEnglish_name() + " "
				+ this.articlePluginBundle.getFieldPlugin(field).getDataType();
	}
	...
	public List<DataField> listByCatId(Integer catid) {
		String sql = "select df.* from "
				+ this.getTableName("data_field")
				+ " df ,"
				+ this.getTableName("data_model")
				+ " dm,"
				+ this.getTableName("data_cat")
				+ " c where df.model_id=dm.model_id and dm.model_id= c.model_id and c.cat_id=?";
	...
	public List<DataField> listIsShow(Integer modelid) {
		...
		return this.baseDaoSupport
				.queryForList(
						"select * from data_field  where model_id=? and is_show=1 order by taxis",
						DataField.class, modelid);
	}
