 ...
 * 数据模型业务类
 * 
 * @author kingapex 2010-7-2下午02:37:26
 */
public class DataModelManager extends BaseSupport<DataModel> implements
		IDataModelManager {
	...
	public void add(DataModel dataModel) {
		...
		createTbSql
				.append(" auto_increment,sort smallint(1) default 0, add_time datetime, lastmodified datetime, hit bigint, able_time");
		createTbSql
				.append(" bigint, state mediumint(8) comment '1:未审核,2:已审核,3:被拒绝',");
		createTbSql
				.append(" user_id bigint, cat_id mediumint(8), is_commend mediumint(4)");
		createTbSql
				.append(" comment '0:普通,1:推荐', sys_lock mediumint(4) default 0 comment '0:正常,1:系统锁定',page_title varchar(255), page_keywords varchar(255), page_description text, site_code int default 100000, siteidlist varchar(255), primary key (id) )type = MYISAM;");
	...
	public void delete(Integer modelid) {
		...
		// 删除模型表
		this.daoSupport.execute("drop table "
				+ this.createTableName(dataModel.getEnglish_name()));

		// 删除模型字段数据记录
		this.baseDaoSupport.execute("delete from data_field where model_id=?",
				modelid);

		// 删除模型记录
		this.baseDaoSupport.execute("delete from data_model where model_id=?",
				modelid);
	...
	public void edit(DataModel dataModel) {
		...
		this.baseDaoSupport.update("data_model", dataModel, "model_id="
				+ dataModel.getModel_id());
	...
	public DataModel get(Integer modelid) {
		...
		return this.baseDaoSupport
				.queryForObject(sql, DataModel.class, modelid);
	}

	public List<DataModel> list() {

		return this.baseDaoSupport.queryForList(
				"select * from data_model order by add_time asc",
				DataModel.class);
	}

	public int checkIfModelInUse(Integer modelid) {
		DataModel dataModel = this.get(modelid);
		return this.daoSupport.queryForInt("select count(0) from "
				+ this.createTableName(dataModel.getEnglish_name()));
	}
