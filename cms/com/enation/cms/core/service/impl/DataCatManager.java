 ...
 * 文章类别管理
 * 
 * @author kingapex 2010-7-5上午07:53:56
 */
public class DataCatManager extends BaseSupport<DataCat> implements
	...
	public void add(DataCat cat) {
		...
		// 判断文章分类不能同名
		if (cat.getName() != null) {
			String sql = "select count(0) from data_cat where name = '"
		...
		sql = "update data_cat set cat_path='" + cat.getCat_path()
				+ "',url='newslist-" + cat_id + "-1.html' where cat_id="
				+ cat_id;
	...
	public void edit(DataCat cat) {
		...
		// 判断文章分类不能同名
		if (cat.getName() != null) {
			String sql = "select count(0) from data_cat where cat_id != "
		...
		this.baseDaoSupport
				.update("data_cat", map, "cat_id=" + cat.getCat_id());
	...
	public DataCat get(Integer catid) {
		...
		return (DataCat) this.baseDaoSupport.queryForObject(
				"select * from data_cat where cat_id=?", DataCat.class, catid);
	}
	...
	public List<DataCat> listAllChildren(Integer parentid) {
		...
		List<DataCat> allCatList = this.baseDaoSupport.queryForList(sql,
				DataCat.class);
		...
		for (DataCat cat : allCatList) {
			if (cat.getParent_id().compareTo(parentid) == 0) {
				...
				List<DataCat> children = this.getChildren(allCatList,
						cat.getCat_id());
	...
	public List<DataCat> getParents(Integer catid) {
		...
		return this.baseDaoSupport.queryForList(
				"select * from data_cat where cat_id in(" + path + ")",
				DataCat.class);
	}
