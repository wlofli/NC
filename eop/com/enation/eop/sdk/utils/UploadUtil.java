...
public class UploadUtil {
	...
	public static String uploadUseRequest(String name, String subFolder) {
		...
		try {
			...
			filePath = EopSetting.IMG_SERVER_PATH
	...
	/**
	 * 上传图片并生成缩略图 图片会被上传至用户上下文的attacment文件夹的subFolder子文件夹中<br/>
	 * 
	 * @param file
	 *            图片file对象
	 * @param fileFileName
	 ...
	 * @param height
	 *            缩略图的高
	 * @return 上传后的图版全路径，如:http://static.eop.com/user/1/1/attachment/goods/
	 *         2001010101030.jpg<br/>
	 *         返回值为大小为2的String数组，第一个元素为上传后的原图全路径，第二个为缩略图全路径
	 */
	public static String[] upload(File file, String fileFileName,
			String subFolder, int width, int height) {
		if (file == null || fileFileName == null)
			throw new IllegalArgumentException(
					"file or filename object is null");
		if (subFolder == null)
		...
		path[0] = EopSetting.IMG_SERVER_DOMAIN + getContextFolder()
				+ "/attachment/" + (subFolder == null ? "" : subFolder) + "/"
				+ fileName;
		filePath += fileName;
		FileUtil.createFile(file, filePath);
	 ...
	 * 
	 * @param filePath
	 *            文件全路径如：http://static.eop.com/user/1/1/attachment/goods/
	 *            2001010101030.jpg
	 */
	 ...
	 * 
	 * @param filePath
	 *            如：http://static.eop.com/user/1/1/attachment/goods/
	 *            2001010101030.jpg
	 * @param shortName
	 * @return
	 */
	...
	public static void main(String[] args) {
		System.out
				.println(getThumbPath(
						"http://static.eop.com/user/1/1/attachment/goods/2001010101030.jpg",
						"_thumbnail"));
		System.out.println(getThumbPath(
				"/user/1/1/attachment/goods/2001010101030.jpg", "_thumbnail"));
	}
