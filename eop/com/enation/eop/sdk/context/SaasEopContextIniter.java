...
public class SaasEopContextIniter {
	public static void init(HttpServletRequest httpRequest,
		...
		if (servletPath.startsWith("/install")
				&& !servletPath.startsWith("/install.html")) {
			site = new EopSite();
			site.setUserid(1);
			site.setId(1);
			site.setThemeid(1);
			...
			ISiteManager siteManager = SpringContextHolder
					.getBean("siteManager");
			...
			if (site.getMulti_site() == 1) { // 开启多站点功能
				IMultiSiteManager multiSiteManager = SpringContextHolder
						.getBean("multiSiteManager");
