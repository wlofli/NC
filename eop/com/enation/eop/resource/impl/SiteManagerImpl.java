...
public class SiteManagerImpl implements ISiteManager {
	...
	public EopSite get(String domain) {
		...
		/*
		 * 这部分为上面几行修改之前的代码，在单机运行下会导致logo路径出错 if (logofile != null) logofile =
		 * logofile.replaceAll(EopSetting.FILE_STORE_PREFIX,
		 * EopSetting.IMG_SERVER_DOMAIN + "/user/" + site.getUserid() + "/" +
		 * site.getId()); // System.out.println(logofile);
		 */
		site.setLogofile(logofile);
		String bklogofile = site.getBklogofile();
		if (bklogofile != null) {
	...
	public Page list(int pageNo, int pageSize, String order, String search) {
		...
		for (Map site : listsite) {
			...
			for (EopSiteDomain siteDomain : listdomain) {
				if (site.get("id").toString()
						.equals(siteDomain.getSiteid().toString())) {
	...
	public void updatePoint(Integer id, int point, int sitestate) {
		this.daoSupport.execute(
				"update eop_site set point=point + ?,sitestate=? where id=?",
				point, sitestate, id);
		this.daoSupport
				.execute(
						"insert into eop_pointLog (siteid, point, dateline) values (?, ?, ?)",
						id, point, DateUtil.getDatelineLong());
	}
	...
	public int getPoint(Integer id, int point) {
		...
		if (now - lastgetpoint > onemonth) {// 已经有一个月未领取积分
			...
			this.daoSupport.execute(
					...
					DateUtil.getDatelineLong(), id); // 更新数据库中最后领取积分时间
	...
	private List<EopApp> listSaasApp() {
		...
		String xmlFile = EopSetting.EOP_PATH
				+ EopContext.getContext().getContextPath() + "/profile.xml";
