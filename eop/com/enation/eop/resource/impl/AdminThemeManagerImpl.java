 ...
 */
public class AdminThemeManagerImpl extends BaseSupport<AdminTheme> implements
		IAdminThemeManager {
	...
	private void copy(AdminTheme theme, boolean isCommon) throws Exception {
		...
		FileUtil.copyFolder(
				...
				EopSetting.EOP_PATH + contextPath + "/adminthemes/"
						+ theme.getPath());
		/*
		 * 只考jsp到eop应用服务器中
		 * 
		 ...
		 * "/adminthemes/" + theme.getPath()) , txtFiles );
		 */
	}

	public AdminTheme get(Integer themeid) {
		List<AdminTheme> list = this.baseDaoSupport.queryForList(
				"select * from admintheme where id=?", AdminTheme.class,
				themeid);
	...
	public List<AdminTheme> list() {
		...
		return this.baseDaoSupport.queryForList("select * from admintheme ",
				AdminTheme.class);
	}
