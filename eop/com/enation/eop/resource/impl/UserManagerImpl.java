 ...
 * 易族易站用户管理
 * 
 * @author kingapex 2010-11-8下午02:22:24
 */
public class UserManagerImpl implements IUserManager {
	...
	public EopUser get(Integer userid) {
		...
		return (EopUser) this.daoSupport.queryForObject(sql, EopUser.class,
				userid);
	}
	...
	public EopUser get(String username) {
		return (EopUser) this.daoSupport.queryForObject(
				"select * from eop_user where username=?", EopUser.class,
				username);
	}
	...
	public void delete(Integer userid) {
		...
		this.daoSupport.execute("delete from eop_userdetail where userid = ?",
				userid); // 删除用户详细表
		this.daoSupport.execute("delete from eop_useradmin where userid = ?",
				userid); // 删除管理员表
