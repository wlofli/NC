 ...
 * @author kingapex
 *         <p>
 *         2009-12-17 上午11:38:56
 *         </p>
 * @version 1.0
 */
public class SiteCacheProxy extends AbstractCacheProxy<EopSite> implements
		ISiteManager {
		...
		if (site == null) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("domain[" + domain
						+ "] not in cache, then query from database");
			}
	...
	public Page list(String keyword, int pageNo, int pageSize) {
		...
		for (Map site : listsite) {
			...
			// logofile.replaceAll(EopSetting.FILE_STORE_PREFIX,EopSetting.IMG_SERVER_DOMAIN+"/user/"+
			// site.get("userid").toString() +"/"+site.get("id").toString());
			// site.put("logofile", logofile);
			for (Dns siteDomain : dnsList) {
				if (site.get("id").toString()
						.equals(siteDomain.getSite().getId().toString())) {
