...
public class ThemeUriCacheProxy extends AbstractCacheProxy<List<ThemeUri>>
	...
	public List<ThemeUri> list() {
		...
		Integer userid = site.getUserid();
		Integer siteid = site.getId();
		List<ThemeUri> uriList = this.cache.get(LIST_KEY_PREFIX + userid + "_"
				+ siteid);

		if (uriList == null) {
			// if(logger.isDebugEnabled()){
			// logger.debug("get user:"+userid+" site: "+ siteid
			// +" theme uri list from database");
			// }
	...
	public ThemeUri getPath(String uri) {
		...
		throw new EopException(uri + "not found 真的not found");
	}

	public static void main(String[] args) {
		/*
		 * Pattern p = Pattern.compile("/goods-(\\d+).html", 2 |
		 * Pattern.DOTALL); Matcher m = p.matcher("/goods-1.html"); String s =
		 * m.replaceAll("/goods.do?id=$1"); System.out.println(s);
		 */
