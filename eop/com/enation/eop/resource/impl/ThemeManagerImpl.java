...
public class ThemeManagerImpl extends BaseSupport<Theme> implements
	...
	public Theme getTheme(Integer themeid) {
		...
		return this.baseDaoSupport.queryForObject(
				"select * from theme where id=?", Theme.class, themeid);
	}

	public List<Theme> list() {
		if (EopContext.getContext().getCurrentSite().getMulti_site() == 0) {
			return this.baseDaoSupport.queryForList(
					"select * from theme where siteid = 0", Theme.class);
		} else {
			return this.baseDaoSupport.queryForList(
					"select * from theme where siteid = ?", Theme.class,
	...
	public void addBlank(Theme theme) {
		try {
			...
			FileUtil.copyFolder(basePath + "/blank/images", targetPath
					+ "/images");
			FileUtil.copyFile(basePath + "/blank/preview.png", targetPath
					+ "/preview.png");
			...
			FileUtil.copyFolder(basePath + "/blank", EopSetting.EOP_PATH
					+ contextPath + "/themes/" + theme.getPath());
	...
	private void copy(Theme theme, boolean isCommon) throws Exception {
		// 公用模板由common目录复制，非公用由产品目录复制
		String basePath = isCommon ? EopSetting.APP_DATA_STORAGE_PATH
		...
		String targetPath = EopSetting.IMG_SERVER_PATH + contextPath
				+ "/themes/" + theme.getPath();
		FileUtil.copyFolder(basePath + "/" + theme.getPath() + "/images",
				targetPath + "/images");
		FileUtil.copyFile(basePath + "/" + theme.getPath() + "/preview.png",
				targetPath + "/preview.png");
		FileUtil.copyFolder(basePath + "/" + theme.getPath() + "/css",
				targetPath + "/css");
		FileUtil.copyFolder(basePath + "/" + theme.getPath() + "/js",
				targetPath + "/js");

		FileUtil.copyFolder(
				basePath + "/" + theme.getPath(),
				EopSetting.EOP_PATH + contextPath + "/themes/"
						+ theme.getPath());
		/*
		 * 只考jsp到eop应用服务器中
		 * 
		 * IOFileFilter txtSuffixFilter =
		 * FileFilterUtils.suffixFileFilter(".jsp"); IOFileFilter txtFiles =
		 * FileFilterUtils.andFileFilter(FileFileFilter.FILE, txtSuffixFilter);
		 * 
		 * FileUtils.copyDirectory( new File(basePath + "/" + theme.getPath() )
		 * ,
		 * 
		 * new File(EopSetting.EOP_PATH + "/user/" +userid + "/" +siteid +
		 * "/themes/" + theme.getPath()) , txtFiles );
		 * 
		 * 
		 * FileUtil.copyFolder(basePath + "/" + theme.getPath(), StringUtil
		 * .getRootPath() + "/user/" +userid + "/" +siteid + "/themes/" +
		 * theme.getPath());
		 */
	}
}
