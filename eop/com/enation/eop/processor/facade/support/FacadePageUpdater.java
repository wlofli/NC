 ...
 * 前台页面更新器
 * 
 * @author kingapex 2010-2-10下午03:54:41
 */
public class FacadePageUpdater implements IPageUpdater {
	...
	public void update(String uri, String pageContent, String paramJson) {
		...
		// 将json字串转为map
		List<Map<String, String>> mapList = WidgetXmlUtil
				.jsonToMapList(paramJson);
		...
		String pagePath = EopSetting.EOP_PATH + contextPath + "/"
				+ EopSetting.THEMES_STORAGE_PATH + "/" + themePath + "/" + uri;
		//
		// String oldContent = FileUtil.read(pagePath, "UTF-8");
		// Pattern p =
		// Pattern.compile("(.*)<div([^<|^>]*?)id=\"AllWrap\"([^<|^>]*?)>(.*)</div>(.*)",
		// 2 | Pattern.DOTALL);
		...
		pageContent = "<#include 'common/header.html' />" + pageContent
