...
public class FacadePagePaser implements IPagePaser {
	...
	public synchronized String pase(String uri) {
		...
		if (!StringUtil.isEmpty(themeUri.getKeywords())
			...
			if (!StringUtil.isEmpty(themeUri.getKeywords()))
				freeMarkerPaser.putData(HeaderConstants.keywords,
						themeUri.getKeywords());

			if (!StringUtil.isEmpty(themeUri.getDescription()))
				freeMarkerPaser.putData(HeaderConstants.description,
						themeUri.getDescription());
		}

		// if(EopSetting.RUNMODE.equals("2")){
		// 计算流量
		// IAccessRecorder accessRecorder =
		// SpringContextHolder.getBean("accessRecorder");
		...
		if (widgets != null) {
			...
			for (String id : idSet) {
				...
				// 对默认页的特殊处理
				if (tplFileName.equals("/default.html") && id.startsWith("/")
						&& !isCurrUrl) {
					continue;
				}
