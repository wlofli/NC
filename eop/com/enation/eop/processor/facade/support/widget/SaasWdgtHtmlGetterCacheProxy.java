...
/**
 * Saas挂件html缓存代理<br>
 * 缓存规则： 以 userid_siteid为key划分出缓存区域，以map作为值。 map中存储此站点的挂件html片断 map
 * 
 * @author kingapex
 * 
 */
public class SaasWdgtHtmlGetterCacheProxy extends
		AbstractCacheProxy<Map<String, String>> implements IWidgetHtmlGetter {
	...
	private boolean getCacheOpen() {
		IWidgetCacheManager widgetCacheManager = SpringContextHolder
				.getBean("widgetCacheManager");
