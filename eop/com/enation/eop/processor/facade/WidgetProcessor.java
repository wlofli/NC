...
public class WidgetProcessor implements Processor {
	...
	public Response process(int mode, HttpServletResponse httpResponse,
		...
		/*
		 * Processor processor =null; Widget widget
		 * =WidgetFactory.build(httpRequest);
		 * 
		 * String act = httpRequest.getParameter("act");
		 * if("delete".equals(act)){ processor =new WidgetDeleter(widget);
		 * }else{ processor =new WidgetGeter(widget); }
		 * 
		 * 
		 * mode = widget.getApp().getDeployment();
		 * 
		 * Response response = processor.process(mode, httpResponse,
		 * httpRequest);
		 * 
		 * return response;
		 */
