...
public class LoginProcessor implements Processor {
	...
	public Response sysLogin(HttpServletResponse httpResponse,
		...
		if (endata == null) {
			response.setContent("非法数据");
			return response;
		}

		endata = EncryptionUtil.authCode(endata, "DECODE");
		String[] ar = endata.split(",");
		// if(ar==null||ar.length!=3){ response.setContent("非法数据"); return
		// response;}
		...
		try {
			 ...
			 * 登录校验
			 */
			IAdminUserManager userManager = SpringContextHolder
					.getBean("adminUserManager");
			...
			response.setContent("{'result':1,'message':'"
	...
	public Response userLogin(HttpServletResponse httpResponse,
		...
		try {
			...
			String remember_login_name = httpRequest
					.getParameter("remember_login_name");
