 ...
 * 默认sql文件执行器
 * 
 * @author kingapex 2010-1-25上午11:50:19
 */
final public class DefaultSqlFileExecutor implements ISqlFileExecutor {
	...
	private void batchExecute(String content, boolean exampleData) {
		...
		/*
		 * if(EopSetting.DBTYPE.equals("2")){ for(int i=0;i<sql_ar.length;i++){
		 * String prefix = sql_ar[i].toLowerCase(); prefix =
		 * prefix.substring(0,7).trim(); if(!"create".equals(prefix)) sql_ar[i]
		 * = sql_ar[i] + ";"; } }
		 */
		...
		if (StringUtil.isEmpty(content) || sql_ar == null || sql_ar.length == 0)
			return;
