 ...
 * 促销规则测试
 * 
 * @author kingapex 2010-4-15下午03:08:44
 */
@RunWith(JMock.class)
public class PromotionTest extends SpringTestSupport {
	...
	public void testAddGiveGift() {
		...
		promotion.setPmt_time_end(DateUtil.toDate("2010-05-20", "yyyy-MM-dd")
				.getTime());
		...
		int count = this.jdbcTemplate
				.queryForInt("select count(0) from js_promotion_2");
		Assert.assertEquals(count, 1);

		// 验证关联了会员
		count = this.jdbcTemplate
				.queryForInt("select count(0) from js_pmt_member_lv_2");
		Assert.assertEquals(count, 2);

		// 验证未关联商品
		count = this.jdbcTemplate
				.queryForInt("select count(0) from js_pmt_goods_2");
	...
	public void testAddDiscount() {
		...
		this.promotionManager.add(promotion, new Integer[] { 1, 2 },
				new Integer[] { 1, 2 });
	...
	public void testAddDiscount2() {
		...
		// //验证插入了一个规则
		// int count =
		// this.jdbcTemplate.queryForInt("select count(0) from js_promotion_2");
		// Assert.assertEquals(count, 1);
		//
		// //验证关联了会员
		// count =
		// this.jdbcTemplate.queryForInt("select count(0) from js_pmt_member_lv_2");
		// Assert.assertEquals(count,2);
		//
		// //验证关联了商品
		// count =
		// this.jdbcTemplate.queryForInt("select count(0) from js_pmt_goods_2");
		// Assert.assertEquals(count,2);
	}

	/**
	 * 测试添加订单满金额就减价规则 满200减5块
	 */
	@Test
	...
	public void testEdit() {
		...
		this.promotionManager.edit(promotion, new Integer[] { 1, 2, 3 },
				new Integer[] { 1, 2, 3 });
	...
	public void testListOrderPmt() {
		...
		Assert.assertEquals(list.get(0).getPmt_describe(),
				"购物车中商品金额大于某金额，赠送某赠品");
	}
	...
	public void testCountPmtPrice() {
		...
		Double shipFree = 10D;
		DiscountPrice dp = promotionManager.applyOrderPmt(orderPrice, shipFree,
				100, 1);
	...
	public static void main(String[] args) {
		...
		System.out.println(DateUtil.toDate("2010-05-20", "yyyy-MM-dd")
				.getTime());
