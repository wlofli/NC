...
public class OrderFlowTest extends OrderTest {
	...
	public void testPay() {
		...
		// 验证支付状态为部分付款
		Assert.assertEquals(order.getPay_status().intValue(),
				OrderStatus.PAY_PARTIAL_PAYED);
		...
		// 验证会员预存款=1000-447
		int biz_money = this.jdbcTemplate
				.queryForInt("select biz_money from js_member_2 where member_id=1");
	...
	public void testRefund() {
		...
		// 验证支付状态为部分退款
		Assert.assertEquals(order.getPay_status().intValue(),
				OrderStatus.PAY_PARTIAL_REFUND);

		// 验证订单状态为退款
		Assert.assertEquals(order.getStatus().intValue(),
				OrderStatus.ORDER_CANCEL_PAY);
	...
	public void shippingTest1() {
		...
		// 验证订单收货状态为部分发货
		Assert.assertEquals(order.getShip_status().intValue(),
				OrderStatus.SHIP_PARTIAL_SHIPED);

		// 验证商品库存为10-1
		int goodsStore = this.jdbcTemplate
				.queryForInt("select store from  js_goods_2 where goods_id=1");
		Assert.assertEquals(goodsStore, 9);
		goodsStore = this.jdbcTemplate
				.queryForInt("select store from js_goods_2 where goods_id=3");
		Assert.assertEquals(goodsStore, 9);

		// 验证货品库存为10-1
		int productStore = this.jdbcTemplate
				.queryForInt("select store from js_product_2 where product_id=2");
		Assert.assertEquals(productStore, 9);
		productStore = this.jdbcTemplate
				.queryForInt("select store from js_product_2 where product_id=11");
		Assert.assertEquals(productStore, 9);

		/************************ 第二次发货，2个商品发货量为1 ******************************/
		/*
		 * itemList = new ArrayList<DeliveryItem>(); DeliveryItem item3= new
		 * DeliveryItem(); item3.setGoods_id(3);
		 * item3.setName("新韩料雅致灰极致瘦腿提臀百搭裙子"); item3.setNum(1);
		 * item3.setProduct_id(11); itemList.add(item3);
		 * 
		 * this.orderFlowManager.shipping(delivery, itemList); order =
		 * this.orderManager.get(1);
		 * 
		 * // 验证订单状态为已经发货
		 * Assert.assertEquals(order.getStatus().intValue(),OrderStatus
		 * .ORDER_SHIP);
		 * 
		 * // 验证订单收货状态为已发货
		 * Assert.assertEquals(order.getShip_status().intValue(),
		 * OrderStatus.SHIP_YES);
		 * 
		 * //验证商品库存为10-1 goodsStore = this.jdbcTemplate.queryForInt(
		 * "select store from  js_goods_2 where goods_id=1");
		 * Assert.assertEquals(goodsStore,9);
		 * 
		 * 
		 * //验证商品库存为10-1-1 goodsStore = this.jdbcTemplate.queryForInt(
		 * "select store from js_goods_2 where goods_id=3");
		 * Assert.assertEquals(goodsStore,8);
		 * 
		 * //验证货品库存为10-1 productStore= this.jdbcTemplate.queryForInt(
		 * "select store from js_product_2 where product_id=2");
		 * Assert.assertEquals(productStore,9);
		 * 
		 * //验证货品库存为10-1-1 productStore= this.jdbcTemplate.queryForInt(
		 * "select store from js_product_2 where product_id=11");
		 * Assert.assertEquals(productStore,8);
		 */
		/************************ 第三次发货，2个商品发货量为1 ******************************/
		/*
		 * itemList = new ArrayList<DeliveryItem>(); DeliveryItem item4= new
		 * DeliveryItem(); item4.setGoods_id(3);
		 * item4.setName("新韩料雅致灰极致瘦腿提臀百搭裙子"); item4.setNum(1);
		 * item4.setProduct_id(11); itemList.add(item3);
		 * 
		 * try{ this.orderFlowManager.shipping(delivery, itemList);
		 * Assert.fail("订单发货状态已经为已发货，不能对其进行发货"); }catch(IllegalStateException
		 * e){
		 * 
		 * }
		 */
	...
	public void shippingTest2() {
		...
		// 验证商品库存为10-2
		goodsStore = this.jdbcTemplate
				.queryForInt("select store from js_goods_2 where goods_id=4");
		Assert.assertEquals(goodsStore, 8);

		// 验证货品库存为10-1
		int productStore = this.jdbcTemplate
				.queryForInt("select store from js_product_2 where product_id=22");
		Assert.assertEquals(productStore, 9);

		// 验证商品库存为10-2
		productStore = this.jdbcTemplate
				.queryForInt("select store from js_product_2 where product_id=28");
		Assert.assertEquals(productStore, 8);
	...
	public void testReturned1() {
		...
		// 验证订单状态为已经退货
		Assert.assertEquals(order.getStatus().intValue(),
				OrderStatus.ORDER_CANCEL_SHIP);

		// 验证订单收货状态为部分退货
		Assert.assertEquals(order.getShip_status().intValue(),
				OrderStatus.SHIP_CANCEL);
