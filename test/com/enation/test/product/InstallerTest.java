 ...
 * 为用户安装product的测试
 * 
 * @author kingapex 2010-1-20下午10:53:54
 */
// @RunWith(JMock.class)
public class InstallerTest extends SpringTestSupport {
	...
	public void installTestForLogined() {
		/*
		 * //清空 this.cleanAdminTheme(); this.cleanMenu(); this.cleanTheme();
		 * this.cleanUri(); this.cleanWidget(); IUserService userService =
		 * UserServiceFactory.getUserService(); String sql
		 * ="truncate table base_menu_"+ userService.getCurrentUserId() ;
		 * this.jdbcTemplate.execute(sql);
		 * 
		 * 
		 * UserDTO userDTO = new UserDTO();
		 * 
		 * EopUser eopUser = new EopUser(); eopUser.setAddress("北京市昌平区");
		 * eopUser.setUsername("易族智汇（北京）科技有限公司"); eopUser.setLinkman("刘志毅");
		 * eopUser.setTel("01061750491"); eopUser.setEmail("enation@126.com");
		 ...
		 * 
		 * /////////////// SiteDTO siteDTO = new SiteDTO(); EopSite site = new
		 * EopSite(); site.setThemeid(1); site.setThemepath("zqzy");
		 * site.setAdminthemeid(1); site.setSitename("测试站点");
		 * siteDTO.setSite(site);
		 * 
		 * EopSiteDomain domain = new EopSiteDomain();
		 * domain.setDomain("kingapex.eop.com"); siteDTO.setDomain(domain);
		 * 
		 ...
		 * 
		 * IUserManager userManager = getBean("userManager"); Integer userid =
		 * userManager.createUser(userDTO);
		 * 
		 * 
		 * 
		 * //为用户安装一个书店product IProductService productService =
		 * this.getBean("productService"); productService.install(userid,
		 * userDTO.getSiteid(), "base"); productService.install(userid,
		 * userDTO.getSiteid(),"bookstore");
		 * 
		 * 
		 * //验证断言 this.adminThemeInstallAssert(); this.menuInstallAssert();
		 * this.uriInstallAssert(); this.themeInstallAssert();
	...
	private void cleanMenu() {
		...
		String sql = "truncate table base_menu_"
	...
	private void cleanWidget() {
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "truncate table base_widgetbundle_"
				+ userService.getCurrentUserId();
		this.jdbcTemplate.execute(sql);
	}
	...
	private void cleanUri() {
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "truncate table base_themeuri_"
				+ userService.getCurrentUserId();
		this.jdbcTemplate.execute(sql);
	}
	...
	private void cleanAdminTheme() {
		// 清除主题相关数据
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "truncate table base_admintheme_"
				+ userService.getCurrentUserId();
		this.jdbcTemplate.execute(sql);
	...
	private void cleanTheme() {
		// 清除主题相关数据
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "truncate table base_theme_"
				+ userService.getCurrentUserId();
		this.jdbcTemplate.execute(sql);
	...
	public void installUriTest() {
		...
		adminThemeInstaller.install("bookstore", proFileDoc
	...
	public void installThemeTest() {
		...
		themeInstaller.install("bookstore",
				proFileDoc.getElementsByTagName("themes").item(0));
	...
	public void installAdminThemeTest() {
		...
		adminThemeInstaller.install("base",
				proFileDoc.getElementsByTagName("adminThemes").item(0));
	...
	public void installMenuTest() {
		...
		menuInstaller.install("bookstore",
				proFileDoc.getElementsByTagName("menu").item(0));
	...
	public void installWidgetTest() {
		...
		menuInstaller.install("bookstore",
				proFileDoc.getElementsByTagName("widgets").item(0));
	...
	/**
	 * 测试菜单是否安装成功的断言 查询数据库是否有记录
	 */
	private void menuInstallAssert() {
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "select count(0) from base_menu_"
				+ userService.getCurrentUserId() + " where siteid=?";
		int count = this.simpleJdbcTemplate.queryForInt(sql,
	...
	private void adminThemeInstallAssert() {
		...
		// 查询数据库是否有记录
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "select count(0) from base_adminTheme_"
				+ userService.getCurrentUserId() + " where siteid=?";
		int count = this.simpleJdbcTemplate.queryForInt(sql,
				userService.getCurrentSiteId());
		assertEquals(true, count > 0);
		...
		file = new File(EopSetting.IMG_SERVER_PATH + "/user/"
				+ userService.getCurrentUserId() + "/"
				+ userService.getCurrentSiteId()
				+ EopSetting.ADMINTHEMES_STORAGE_PATH + "/dede");
		assertEquals(true, file.exists());
	...
	private void themeInstallAssert() {
		...
		// 查询数据库是否有记录
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "select count(0) from base_theme_"
				+ userService.getCurrentUserId() + " where siteid=?";
		int count = this.simpleJdbcTemplate.queryForInt(sql,
				userService.getCurrentSiteId());
		assertEquals(true, count > 0);
	...
	private void uriInstallAssert() {
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "select count(0) from base_themeuri_"
				+ userService.getCurrentUserId() + " where siteid=?";
		int count = this.simpleJdbcTemplate.queryForInt(sql,
				userService.getCurrentSiteId());
		assertEquals(true, count > 0);
	...
	private void widgetInstallAssert() {
		IUserService userService = UserServiceFactory.getUserService();
		String sql = "select count(0) from base_widgetbundle_"
				+ userService.getCurrentUserId() + " where siteid=?";
		int count = this.simpleJdbcTemplate.queryForInt(sql,
				userService.getCurrentSiteId());
		assertEquals(true, count > 0);
	...
	private String getDedePath() {
		IUserService userService = UserServiceFactory.getUserService();
		return EopSetting.EOP_PATH + "/user/" + userService.getCurrentUserId()
				+ "/" + userService.getCurrentSiteId()
				+ EopSetting.ADMINTHEMES_STORAGE_PATH + "/dede";
	}
	...
	private String getThemePath() {
		...
		return EopSetting.EOP_PATH + "/user/" + userService.getCurrentUserId()
				+ "/" + userService.getCurrentSiteId()
				+ EopSetting.THEMES_STORAGE_PATH + "/default";
	}
