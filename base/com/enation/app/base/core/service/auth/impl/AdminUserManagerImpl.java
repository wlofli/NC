...
public class AdminUserManagerImpl extends BaseSupport<AdminUser> implements
	...
	public void edit(AdminUser adminUser) {
		...
		// 给用户赋予角色
		permissionManager.giveRolesToUser(adminUser.getUserid(),
				adminUser.getRoleids());
	...
	public AdminUser get(Integer id) {
		return this.baseDaoSupport.queryForObject(
				"select * from adminuser where userid=?", AdminUser.class, id);
	}

	public List list() {
		return this.baseDaoSupport.queryForList(
				"select * from adminuser order by dateline", AdminUser.class);
	}
		...
		if (site.getMulti_site() == 1) {
			if (user.getFounder() != 1) { // 非超级管理员检测是否是站点管理员
				...
				if (user.getSiteid() == null
						|| childSite.getSiteid() != user.getSiteid()) {
