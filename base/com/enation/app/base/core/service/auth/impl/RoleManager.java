 ...
 * 角色管理
 * 
 * @author kingapex 2010-10-24下午11:08:12
 */
public class RoleManager extends BaseSupport<Role> implements IRoleManager {
	...
	public void add(Role role, int[] authids) {
		...
		for (int authid : authids) {
			this.baseDaoSupport.execute(
					"insert into role_auth(roleid,authid)values(?,?)", roleid,
					authid);
		}
	...
	public void delete(int roleid) {
		...
		// 删除用户角色
		this.baseDaoSupport.execute("delete from user_role where roleid=?",
				roleid);

		// 删除角色权限
		this.baseDaoSupport.execute("delete from role_auth where roleid =?",
				roleid);
	...
	public void edit(Role role, int[] authids) {
		...
		if (StringUtil.isEmpty(role.getRolename()))
			throw new IllegalArgumentException("编辑角色时名称不可为空");
	...
	public List<Role> list() {
		...
		return this.baseDaoSupport.queryForList("select * from role",
				Role.class);
	...
	public Role get(int roleid) {
		String sql = "select * from " + this.getTableName("auth_action")
				+ " where actid in(select authid from "
				+ this.getTableName("role_auth") + " where roleid =?)";
		List actlist = this.daoSupport.queryForList(sql, AuthAction.class,
				roleid);
		Role role = this.baseDaoSupport.queryForObject(
				"select * from role where roleid=?", Role.class, roleid);
