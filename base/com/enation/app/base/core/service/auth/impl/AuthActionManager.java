 ...
 */
public class AuthActionManager extends BaseSupport<AuthAction> implements
		IAuthActionManager {
	...
	public void delete(int actid) {
		// 删除角色权限表中对应的数据
		this.baseDaoSupport.execute("delete from role_auth where authid=?",
				actid);
		// 删除权限基本数据
		this.baseDaoSupport.execute("delete from auth_action where actid=?",
				actid);
	}

	public void edit(AuthAction act) {
		this.baseDaoSupport.update("auth_action", act,
				"actid=" + act.getActid());
	}

	public List<AuthAction> list() {

		return this.baseDaoSupport.queryForList("select * from auth_action",
				AuthAction.class);
	}

	public AuthAction get(int authid) {
		return this.baseDaoSupport.queryForObject(
				"select * from auth_action where actid=?", AuthAction.class,
				authid);
	}
