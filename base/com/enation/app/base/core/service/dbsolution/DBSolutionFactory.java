...
public class DBSolutionFactory {
	public static IDBSolution getDBSolution() {
		...
		/*
		 * if (result != null) { JdbcTemplate jdbcTemplate = SpringContextHolder
		 * .getBean("jdbcTemplate"); try { DataSource dataSource =
		 * jdbcTemplate.getDataSource(); Connection conn =
		 * dataSource.getConnection(); result.setConnection(conn); } catch
