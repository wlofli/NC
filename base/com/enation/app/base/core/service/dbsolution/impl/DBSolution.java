...
public abstract class DBSolution implements IDBSolution {
	...
	protected String getTableName(String table) {
		/**
		 * 下列代码已经移至StringUtils中 String result = ""; if (table.length() >
		 * prefix.length()) { if (table.substring(0,
		 * prefix.length()).toLowerCase() .equals(prefix.toLowerCase())) result
		 * = table; else result = prefix + table; } else result = prefix +
		 * table;
		 * 
		 * return result;
		 */
