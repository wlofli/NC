...
public class AccessExporter implements Runnable {
	...
	public void run() {
		...
		InputStream in = FileUtil
				.getResourceAsStream("com/enation/eop/resource/access.xls");
		excelUtil.openModal(in);

		// 查询某站点上个月的流量记录
		String[] lastMonth = DateUtil.getLastMonth(); // 得到上个月第一天和最后一天的字串数组
		int start = (int) (DateUtil.toDate(lastMonth[0], "yyyy-MM-dd")
				.getTime() / 1000); // 上个月第一天的秒数
		int end = (int) (DateUtil.toDate(lastMonth[1], "yyyy-MM-dd").getTime() / 1000); // 上个月最后一天的秒数

		String sql = "select * from "
				+ tablename
				+ " where access_time>=? and access_time<=? order by access_time";
		List<Access> list = this.daoSupport.queryForList(sql, Access.class,
				start, end);
		...
		if (list != null && !list.isEmpty()) {
			...
			for (Access access : list) {
				...
				excelUtil.writeStringToCell(i, 4, DateUtil.toString(new Date(
						(long) access.getAccess_time() * 1000),
						"yyyy-MM-dd hh:mm:ss")); // 访问日期
			...
			excelUtil.writeToFile(target + "/access"
					+ lastMonth[0].replaceAll("-01", "") + ".xls");

			// 更新站点累计消费积分
			sql = "select sum(point) point  from " + tablename
					+ " where access_time>=? and access_time<=?";
			Long sumpoint = (Long) this.daoSupport
					.queryForLong(sql, start, end);
			this.daoSupport.execute(
					"update eop_site set sumpoint=sumpoint+?  where id=?",
					sumpoint, this.siteid);

			// 更新站点累计访问量
			sql = "select count(0) c  from " + tablename
					+ " where access_time>=? and access_time<=?";
			Long sumaccess = (Long) this.daoSupport.queryForLong(sql, start,
					end);
			this.daoSupport.execute(
					"update eop_site set sumaccess=sumaccess+?  where id=?",
					sumaccess, this.siteid);

			// 删除导出的数据
			this.daoSupport.execute("delete  from " + tablename
					+ " where access_time>=? and access_time<=?", start, end);
		}
	}
	...
	public static void main(String[] args) {
		...
		System.out
				.println(DateUtil.toDate(date, "yyyy-MM-dd").getTime() / 1000);
	}
