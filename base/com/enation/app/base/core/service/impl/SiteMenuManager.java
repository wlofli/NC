...
public class SiteMenuManager extends BaseSupport<SiteMenu> implements
		ISiteMenuManager {
	...
	public void edit(SiteMenu siteMenu) {
		this.baseDaoSupport.update("site_menu", siteMenu,
				"menuid=" + siteMenu.getMenuid());
	}

	public List<SiteMenu> list(Integer parentid) {
		String sql = "select * from site_menu order by parentid,sort";
		List<SiteMenu> menuList = this.baseDaoSupport.queryForList(sql,
				SiteMenu.class);
		...
		for (SiteMenu menu : menuList) {
			if (menu.getParentid().compareTo(parentid) == 0) {
				if (this.logger.isDebugEnabled()) {
					this.logger.debug("发现子[" + menu.getName() + "-"
							+ menu.getMenuid() + "]");
				}
				List<SiteMenu> children = this.getChildren(menuList,
						menu.getMenuid());
	...
	private List<SiteMenu> getChildren(List<SiteMenu> menuList, Integer parentid) {
		...
		for (SiteMenu menu : menuList) {
			if (menu.getParentid().compareTo(parentid) == 0) {
				if (this.logger.isDebugEnabled()) {
					this.logger.debug(menu.getName() + "-" + menu.getMenuid()
							+ "是子");
				}
	...
	public void updateSort(Integer[] menuid, Integer[] sort) {
		...
		if (menuid == null || sort == null)
			throw new IllegalArgumentException("menuid or sort is NULL");
		if (menuid.length != sort.length)
			throw new IllegalArgumentException("menuid or sort length not same");
		for (int i = 0; i < menuid.length; i++) {
			this.baseDaoSupport.execute(
					"update site_menu set sort=? where menuid=?", sort[i],
					menuid[i]);
		}
	...
	public SiteMenu get(Integer menuid) {
		return this.baseDaoSupport.queryForObject(
				"select * from site_menu where menuid=?", SiteMenu.class,
				menuid);
	}
