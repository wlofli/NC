...
public class AdColumnManager extends BaseSupport<AdColumn> implements
	...
	public AdColumn getADcolumnDetail(Long acid) {
		AdColumn adColumn = this.baseDaoSupport.queryForObject(
				"select * from adcolumn where acid = ?", AdColumn.class, acid);
		return adColumn;
	}

	public List listAllAdvPos() {
		List<AdColumn> list = this.baseDaoSupport.queryForList(
				"select * from adcolumn", AdColumn.class);
		return list;
	}
	...
	public void updateAdvc(AdColumn adColumn) {
		this.baseDaoSupport.update("adcolumn", adColumn,
				"acid = " + adColumn.getAcid());
	}
