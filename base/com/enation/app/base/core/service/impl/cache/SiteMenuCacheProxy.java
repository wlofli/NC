...
public class SiteMenuCacheProxy extends AbstractCacheProxy<List<SiteMenu>>
	...
	private void cleanCache() {
		...
		this.cache.remove(MENU_LIST_CACHE_KEY + "_" + site.getUserid() + "_"
				+ site.getId());
	}
	...
	public List<SiteMenu> list(Integer parentid) {
		EopSite site = EopContext.getContext().getCurrentSite();
		List<SiteMenu> menuList = this.cache.get(MENU_LIST_CACHE_KEY + "_"
				+ site.getUserid() + "_" + site.getId());

		if (menuList == null) {
			menuList = this.siteMenuManager.list(parentid);
			this.cache.put(MENU_LIST_CACHE_KEY + "_" + site.getUserid() + "_"
					+ site.getId(), menuList);
