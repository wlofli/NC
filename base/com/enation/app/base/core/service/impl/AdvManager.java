...
public class AdvManager extends BaseSupport<Adv> implements IAdvManager {
	...
	public Adv getAdvDetail(Long advid) {
		Adv adv = this.baseDaoSupport.queryForObject(
				"select * from adv where aid = ?", Adv.class, advid);
	...
	public List listAdv(Long acid) {
		...
		List<Adv> list = this.baseDaoSupport
				.queryForList(
						"select a.*,'' cname from adv a where acid = ? and begintime<=? and endtime>=? and isclose = 0",
						new AdvMapper(), acid, nowtime, nowtime);
		return list;
	}

	@Override
	public Page search(Long acid, String cname, int pageNo, int pageSize,
			String order) {
