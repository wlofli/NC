...
public class UpdateManager implements IUpdateManager {
		...
		if (response != null) {
			...
			if (!StringUtil.isEmpty(newVersion)) {
				...
				if (this.versionLargerThen(newVersion, currentVersion)) {
					...
					response = request.execute(serviceDomain
							+ "/lastUpdate.txt");
	...
	public void update() {
		...
		if (versionState.getHaveNewVersion()) {
			...
			String serviceDomain = EopSetting.SERVICE_DOMAIN_NAME;
			Request request = new RemoteRequest();
			Response response = request.execute(serviceDomain
					+ "/patch/update/");
	...
	private void download(String zipName) throws Exception {
		...
		URL url = new URL(EopSetting.SERVICE_DOMAIN_NAME + "/patch/update/"
				+ zipName);
	...
	private void unZip(String zipName) {
		...
		FileUtil.unZip(
				zipPath,
				EopSetting.EOP_PATH + "/patch/"
						+ zipName.replaceAll(".zip", ""), true);
	 ...
	 */
	public List<String> sortVersionList(String currentVersion,
			List<String> zipList) {
		...
		for (String n : zipList) {
			String[] num = n.split("\\.");
			Integer result = Integer.valueOf(num[0]) * 1000000
					+ Integer.valueOf(num[1]) * 1000 + Integer.valueOf(num[2]);
			Map m = new HashMap();
			m.put("string", n);
			m.put("result", result);
			lst.add(m);
		...
		for (int i = 0; i < lst.size(); i++) {
			for (int j = i; j < lst.size(); j++) {
				if (Integer.valueOf(lst.get(i).get("result").toString()) > Integer
						.valueOf(lst.get(j).get("result").toString())) {
					Map m = new HashMap();
			...
			if (versionLargerThen(lst.get(i).get("string").toString(),
					currentVersion))
