...
public class SqlExportService {
	...
	public String dumpSql(List<String> tables, String prefix) {
		...
		for (int i = 0; i < tables.size(); i++) {
			...
			if (tabname.startsWith("eop_"))
				continue;
			if (tabname.endsWith("widgetbundle"))
				continue;

			if (tabname.endsWith("border"))
				continue;
			if (tabname.endsWith("menu") && !tabname.endsWith("site_menu"))
				continue;
			if (tabname.endsWith("themeuri"))
				continue;
			if (tabname.endsWith("theme"))
				continue;
			if (tabname.endsWith("admintheme"))
				continue;
			String querySql;

			if ("2".equals(EopSetting.RUNMODE)) {
				EopSite site = EopContext.getContext().getCurrentSite();
				querySql = "show tables like '" + prefix + tabname + "_"
	...
	public String dumpSql(List<String> tables, String target, Document setup) {
		...
		for (int i = 0; i < tables.size(); i++) {
			...
			if ("2".equals(EopSetting.RUNMODE)) {
				EopSite site = EopContext.getContext().getCurrentSite();
				querySql = "show tables like 'es_" + tabname + "_"
						+ site.getUserid() + "_" + site.getId() + "'";
			} else {
			...
			if (tabname.toLowerCase().equals("data_cat")
					|| tabname.toLowerCase().equals("data_model")
					|| tabname.toLowerCase().equals("data_field")) {
	 ...
	 * @return
	 */
	public String dumpTableSql(String table) throws BadSqlGrammarException,
			EmptyResultDataAccessException {
		// System.out.println("备份：" + table);

		// //没有主键的表，不能使用 identity_insert on
		// String[] no_key_tables= new
		// String[]{"es_goods_spec","es_goods_complex","es_tag_rel","es_type_brand"
		// ,"es_pmt_member_lv","es_pmt_goods"
		// ,"es_order_pmt","es_dly","es_dly_type_area","es_order_gift","es_member_coupon","es_package_product"
		// };
		//
		String sql = "";// "truncate table "+
						// table+"_<userid>_<siteid>;\n";//"DROP TABLE IF EXISTS "
						// + table + ";\n";
	...
	public String getTableStatus(String table)
		...
		String sql = "";
		Map map = simpleJdbcTemplate.queryForMap("SHOW TABLE STATUS LIKE '"
				+ table + "'");
		if (map != null) {
			if (map.get("Auto_increment") != null
					&& !map.get("Auto_increment").equals("")) {
				sql = "  AUTO_INCREMENT="
						+ map.get("Auto_increment").toString();
			}
		}
		sql += ";\n\n";
		return sql;
	...
	public String getInsertSql(final String table)
		...
		int total = simpleJdbcTemplate.queryForInt("SELECT COUNT(0) FROM "
				+ rname);
		int pageSize = 200;
				...
				for (int i = 1; i <= count; i++) {
					...
					if (value != null) {
						valuestr.append(comma
								+ getValueString(mysql_escape_string(value),
										type));
					} else {
		...
		for (int i = 1; i <= pageTotal; i++) {
			querysql = "SELECT * FROM " + rname + " LIMIT "
