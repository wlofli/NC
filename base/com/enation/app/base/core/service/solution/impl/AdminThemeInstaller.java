...
public class AdminThemeInstaller implements IInstaller {
	...
	public void install(String productId, Node fragment) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("user install admintheme[" + fragment + "] from "
					+ productId);
		}

		if (fragment == null)
			throw new RuntimeException("install admintheme error[node is null]");
	...
	private void install(Element themeNode, String productId) {
		...
		Document iniFileDoc = adminThemeInfoFileLoader.load(productId, path,
				isCommonTheme);
		Node themeN = null;
		try {
			themeN = iniFileDoc.getFirstChild();
			if (themeN == null) {
				throw new RuntimeException("adminthem node is null");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			if (list == null || list.item(0) == null) {
				throw new RuntimeException("author node is null");
			}
