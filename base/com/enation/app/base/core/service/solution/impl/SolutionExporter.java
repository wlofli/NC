...
public class SolutionExporter implements ISolutionExporter {
	...
	public void export(String name, boolean exportData, boolean exportTheme,
		...
		// 导出文件要存放的位置
		String datapath = EopSetting.IMG_SERVER_PATH + "/"
		...
		try {
			...
			if (exportData) {
				...
				// 生成setup.xml
				org.dom4j.Document setup = setupCreator.createSetup(temppath
						+ "/setup.xml");
				...
				xmlFile.append("</dbsolution>");
				if ("1".equals(EopSetting.DBTYPE)) {
					FileUtil.write(temppath + "/example_data_mysql.sql",
							sqlContent.toString());
				}
				setupCreator.save(setup, temppath + "/setup.xml");
				FileUtil.write(temppath + "/example_data.xml",
						xmlFile.toString());
			}
			...
			if (exportTheme) {
				...
				if ("1".equals(EopSetting.RESOURCEMODE)) { // 静态资源分离
					FileUtil.copyFolder(EopSetting.EOP_PATH
							+ EopContext.getContext().getContextPath()
							+ "/themes", temppath + "/themes");
					FileUtil.copyFolder(EopSetting.IMG_SERVER_PATH
							+ EopContext.getContext().getContextPath()
							+ "/themes", temppath + "/themes");
				}
			...
			if (exportAttr) {
				...
				FileUtil.copyFolder(EopSetting.IMG_SERVER_PATH
						+ EopContext.getContext().getContextPath()
						+ "/attachment", temppath + "/attachment");
			}
