...
public class ProductColorService extends BaseSupport<ProductColor> implements
	...
	public void add(ProductColor color) {
		if (StringUtil.isEmpty(color.getColorname()))
			throw new IllegalArgumentException(
					"argument  color.colorname is null");
	...
	public void edit(ProductColor color) {
		if (color.getId() == 0)
			throw new IllegalArgumentException("argument  color.id is null");
		if (StringUtil.isEmpty(color.getColorname()))
			throw new IllegalArgumentException(
					"argument  color.colorname is null");
		this.daoSupport.update("eop_product_color", color,
				"id=" + color.getId());
	...
	public ProductColor get(int id) {
		return this.daoSupport.queryForObject(
				"select * from eop_product_color where id=?",
				ProductColor.class, id);
	}
