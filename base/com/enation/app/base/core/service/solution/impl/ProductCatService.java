 ...
 * 解决方案分类管理实现
 * 
 * @author kingapex 2010-9-1下午01:55:23
 */
public class ProductCatService extends BaseSupport<ProductCat> implements
		IProductCatService {

	public void add(ProductCat cat) {
		if (StringUtil.isEmpty(cat.getName()))
			throw new IllegalArgumentException("argument  cat.name is null");
		cat.setNum(0);
	...
	public void edit(ProductCat cat) {
		if (cat.getId() == 0)
			throw new IllegalArgumentException("argument  cat.id is null");
		if (StringUtil.isEmpty(cat.getName()))
			throw new IllegalArgumentException("argument  cat.name is null");
	...
	public ProductCat get(int id) {
		...
		return this.daoSupport.queryForObject(
				"select * from eop_product_cat where id=?", ProductCat.class,
				id);
	}
