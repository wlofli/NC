 ...
 * 产品(解决方案)业务逻辑
 * 
 * @author kingapex 2010-1-21上午10:58:12
 */
public class ProductServiceImpl implements IProductService {
	...
	public EopProduct getProduct(Integer id) {
		return (EopProduct) daoSupport.queryForList(
				"select * from eop_product where id = ?", new ProductMapper(),
				id).get(0);
	...
	/*
	 * 暂时废弃 - 2011-08-13 by Liuzy public Page pageProduct(Integer catid,Integer
	 * typeid, Integer state, String order, int page, int pageSize) { order =
	 * order == null ? " id desc" : order; String sql =
	 * "select * from eop_product";
	 * 
	 * if(catid!=null){ sql+=" where catid="+catid; }
	 * 
	 * if(typeid!=null){ sql+= " where typeid="+ typeid; }
	 * 
	 * sql += " order by  " + order;
	 * 
	 * Page webpage = this.daoSupport.queryForPage(sql,page, pageSize, new
	 * ProductMapper()); // Page webpage = this.daoSupport.queryForPage(sql,
	 * page, pageSize); return webpage; }
	 */
	 ...
	 * 
	 * @param product
	 *            解决方案实体，其id属性和名称属性不能为空，否则将会抛出 {@link IllegalArgumentException}
	 * @param zipPath
	 *            解决方案zip包文件绝对路径，添加时不能为空，否则将会抛出 {@link IllegalArgumentException}
	 ...
	 *            解决方案实体，其id属性和名称属性不能为空，否则将会抛出 {@link IllegalArgumentException}
	 * @param zipPath
	 *            解决方案zip包文件绝对路径, 当zipPath为空时则只更新解决方案的基本信息。
	...
	public void update(EopProduct product, String zipPath) {
		...
		this.daoSupport.update("eop_product", product,
				"productid='" + product.getProductid() + "'");
	}

	/**
	 * 删除解决方案<br/>
	 * 注意！此处不再修改product_cat表中的数量，而是在数据库中以触发器的形式解决 将会删除数据库信息和解决方案文件
	 * 
	 * @param productid
	 *            解决方案id
	...
	public Page pageProduct(Integer catid, Integer colorid, Integer typeid,
			Integer state, String order, int page, int pageSize) {
