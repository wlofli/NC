...
public class AdvAction extends WWAction {
	...
	public String list() {
		...
		this.webpage = advManager.search(acid, advname, this.getPage(),
				this.getPageSize(), this.order);
		return "list";
	}
	...
	public String click() {
		if (advid == 0) {
			...
			if (this.getRequest().getSession()
					.getAttribute("AD" + advid.toString()) == null) {
				this.getRequest().getSession()
						.setAttribute("AD" + advid.toString(), "clicked");
	...
	public String addSave() {
		if (pic != null) {
			...
			if (FileUtil.isAllowUp(picFileName)) {
				String path = UploadUtil.upload(this.pic, this.picFileName,
						"adv");
				adv.setAtturl(path);
			} else {
