...
public abstract class UrlUtils {
	...
	// 用于搜索的字段名字
	private static String[] searchFields = new String[] { "cat", "brand",
			"price", "sort", "prop", "circlar", "capacity", "color",
			"diameter", "jpgn", "mate", "moisture", "nattr", "refractive",
			"tyjgn", "page", "tag" };
	...
	public static String appendParamValue(String url, String name, String value) {
		...
		if (old_value != null) {
			if ("prop".equals(name)) { // 对于属性字串的特殊处理
				old_value = old_value.replaceAll(value.split("_")[0]
						+ "_(\\d+)", "");
