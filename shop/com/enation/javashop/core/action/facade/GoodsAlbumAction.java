...
public class GoodsAlbumAction extends WWAction {
	...
	public String execute() {
		...
		String image_file = goods.getImage_file() == null ? "" : goods
				.getImage_file().toString();
		albumXml = new StringBuffer("<products name='" + goods.getName()
				+ "' shopname='Javashop'>");
		if (image_file != null && !image_file.equals("")) {
			...
			for (String img : imgs) {
				...
				if (i == 0)
					albumXml.append(" selected='selected'");
				albumXml.append(">");
				albumXml.append(UploadUtil.getThumbPath(img, "_big")
						+ "</bigpic>");
