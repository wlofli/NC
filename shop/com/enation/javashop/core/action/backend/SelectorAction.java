 ...
 * 商品/货品/赠品选择器
 * 
 * @author kingapex 2010-4-22上午09:34:57
 */
public class SelectorAction extends WWAction {
	...
	public String goods() {
		...
		this.webpage = this.goodsManager.searchGoods(null, name, sn, null,
				null, order, this.getPage(), 5);
		return "goods";
	}
	...
	public String gift() {
		if (keyword != null)
			keyword = StringUtil.toUTF8(keyword);
		this.webpage = this.freeOfferManager.list(keyword, order,
				this.getPage(), 3);
		return "gift";
	}

	/**
	 * 根据一批商品id查询商品列表，并成json数组。 一般供选择完成后形成商品列表使用
	 * 
	 * @return
	 */
	...
	/**
	 * 根据一批商品id查询货品列表，并成json数组。 一般供选择完成后形成货品列表使用
	 * 
	 * @return
	 */
	...
	/**
	 * 根据一批赠品id读取赠品列有，并形成json数组 一般供选择完成后形成赠 品列表使用
	 * 
	 * @return
	 */
