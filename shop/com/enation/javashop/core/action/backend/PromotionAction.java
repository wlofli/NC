 ...
 * 促销规则管理
 * 
 * @author kingapex 2010-4-21上午11:35:55
 */
	...
	public String select_condition() {
		...
		try {
			...
			if (pmtid != null) { // 编辑时使用库中读取的开始、结束时间
				...
				// 使关联的会员级别id选中
				List<Integer> dbLvList = this.promotionManager
						.listMemberLvId(pmtid);
			...
			IPromotionMethod pmtMethod = SpringContextHolder
	...
	public String save() {
		...
		try {
			...
			if (pmtid != null) {
				...
				if (this.activityid == null) { // 优惠卷
					Coupons coupons = (Coupons) ThreadContextHolder
							.getSessionContext().getAttribute("coupons");
				...
				pmtid = this.promotionManager.add(promotion, lvidArray,
						goodsidArray);
			...
			if (this.activityid == null) {
				...
				this.urls.put("此活动促销规则列表", "promotion!list.do?activityid="
						+ promotion.getPmta_id());
			}
