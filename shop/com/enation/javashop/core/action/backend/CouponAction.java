...
public class CouponAction extends PromotionAction {
	...
	public String saveAdd() {
		...
		ThreadContextHolder.getSessionContext()
		 ...
		 * try {
		 * 
		 * couponManager.add(coupons); this.msgs.add("优惠卷添加成功"); } catch
		 * (Exception e) { this.msgs.add("优惠卷添加失败"); e.printStackTrace(); }
		 * this.urls.put("优惠卷列表", "#");
		 */
		// return this.MESSAGE;
	}
	...
	public String list() {
		this.webpage = couponManager.list(this.getPage(), this.getPageSize(),
				order);
		return "list";
	}
	...
	public String exlist() {
		this.webpage = couponManager.listExchange(this.getPage(),
				this.getPageSize());
		return "exlist";
	}
