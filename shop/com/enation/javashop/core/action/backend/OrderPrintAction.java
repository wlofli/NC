...
public class OrderPrintAction extends WWAction {
	...
	public String ship_prnt_step2() {
		...
		String prt_tmpl_data = processTmplData(printTmpl.getPrt_tmpl_data(),
				order, dlyCenter);
		this.printTmpl.setPrt_tmpl_data(prt_tmpl_data);
		return "ship_prnt_step2";
	}

	private String processTmplData(String tmplData, Order order,
			DlyCenter dlyCenter) {
		Date date = new Date();
		...
		result = result.replaceAll("发货人-地区", dlyCenter.getProvince() + "-"
				+ dlyCenter.getCity() + "-" + dlyCenter.getRegion());
		...
		result = result.replaceAll("订单总金额",
				StringUtil.toCurrency(order.getOrder_amount()));
		result = result.replaceAll("订单费用金额",
				StringUtil.toCurrency(order.getShipping_amount()));
		result = result.replaceAll("订单物品总重量",
				StringUtil.toString(order.getWeight()));
		result = result.replaceAll("订单-物品数量",
				StringUtil.toString(order.getGoods_num()));
		result = result.replaceAll("订单-备注", order.getRemark());
		result = result.replaceAll("订单-送货时间",
				order.getShip_day() + order.getShip_time());
		result = result.replaceAll("网店名称", EopContext.getContext()
				.getCurrentSite().getSitename());
		return result;
	}
