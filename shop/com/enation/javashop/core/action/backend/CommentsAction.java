...
public class CommentsAction extends WWAction {
	...
	public String list() {
		...
		this.webpage = commentsManager.pageComments_Display(this.getPage(),
				Integer.valueOf(pageSize_com), object_id, object_type,
				commentType);
	...
	public String detail() {
		...
		if (commentDTO.getComments().getCommenttype().trim().equals("goods")) {
			this.goods = this.goodsManager.getGoods(commentDTO.getComments()
					.getObject_id());
			this.target = "相关商品：" + goods.getName();
		} else {
			this.article = this.articleManager.get(commentDTO.getComments()
					.getObject_id());
	...
	public String add() {
		...
		try {
			if ("1".equals(validcode)) { // 如果启用了验证码
				Object realCode = ThreadContextHolder.getSessionContext()
						.getAttribute(
								ValidCodeServlet.SESSION_VALID_CODE
										+ "comments");
			...
			if (managerid != null) {// 判断是否后台已登录
				member.setMember_id(UserServiceFactory.getUserService()
						.getCurrentManagerId());
				member.setName("管理员");
				try {
					Comments parentcomments = commentsManager.getComments(
							comments.getFor_comment_id()).getComments();
			...
			datalog.setContent("标题:" + comments.getTitle() + "<br>内容"
					+ comments.getAcomment());
	...
	public String show() {
		try {
			...
			if (memberPointManger.checkIsOpen(type)) {
				if (comments.getAuthor_id() != null
						&& comments.getAuthor_id().intValue() != 0) {
					int point = memberPointManger.getItemPoint(type + "_num");
					memberPointManger.add(comments.getAuthor_id(), point, type,
							null);
				}
