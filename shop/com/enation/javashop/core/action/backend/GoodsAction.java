...
public class GoodsAction extends WWAction {
	...
	public String list() {
		...
		this.webpage = goodsManager.searchGoods(catid, name, sn, market_enable,
				tagids, order, this.getPage(), this.getPageSize());
	...
	public String edit() {
		...
		fieldList = this.goodsFieldManager.list(Integer.valueOf(""
				+ editDTO.getGoods().get("type_id")));
		for (GoodsField field : fieldList) {
			String html = this.goodsFieldPluginBundle.onDisplay(field,
					editDTO.getGoods());
