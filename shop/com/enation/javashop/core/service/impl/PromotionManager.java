 ...
 * 优惠规则管理
 * 
 * @author kingapex 2010-4-15下午05:14:57
 */
...
public class PromotionManager extends BaseSupport implements IPromotionManager {
	...
	public Integer add(Promotion promotion, Integer[] memberLvIdArray,
		...
		if (promotion == null)
			throw new IllegalArgumentException("param promotion is NULL");
		if (promotion.getPmt_describe() == null)
			throw new IllegalArgumentException(
					"param promotion 's Pmt_describe is NULL");
		if (promotion.getPmts_id() == null)
			throw new IllegalArgumentException(
					"param promotion 's Pmts_id is NULL");
		if (promotion.getPmt_time_begin() == null)
			throw new IllegalArgumentException(
					"param promotion 's pmt_time_begin is NULL");
		if (promotion.getPmt_time_end() == null)
			throw new IllegalArgumentException(
					"param promotion 's Pmt_time_end is NULL");
		// if(promotion.getPmt_basic_type()== null ) throw new
		// IllegalArgumentException("param promotion 's Pmt_basic_type is NULL");
	...
	public Integer add(Promotion promotion, Integer[] memberLvIdArray,
			int type, Integer[] goodsIdArray, Integer[] goodsCatIdArray,
			Integer[] tagIdArray) {
		Integer result = null;
		if (type == 1) {
	...
	public void edit(Promotion promotion, Integer[] memberLvIdArray,
			Integer[] goodsIdArray) {
		...
		if (memberLvIdArray != null) {
			this.baseDaoSupport.execute(
					"delete from pmt_member_lv where pmt_id=?", pmtid);
			this.saveMemberLv(pmtid, memberLvIdArray);
		}

		if (goodsIdArray != null) {
			this.baseDaoSupport.execute("delete from pmt_goods where pmt_id=?",
					pmtid);
			this.saveGoods(pmtid, goodsIdArray);
		}
	...
	public List<Promotion> list(Integer goodsid, Integer memberLvId) {
		...
		if (memberLvId == null)
			throw new IllegalArgumentException("memberLvId is NULL");
		long now = System.currentTimeMillis();
		String sql = "select * from  " + this.getTableName("promotion")
				+ " where pmt_basic_type='goods'" + " and  pmt_time_begin<"
				+ now + " and  pmt_time_end>" + now
				+ " and pmt_id in  (select pmt_id from "
				+ this.getTableName("pmt_goods") + " where goods_id=?)"
				+ " and pmt_id in (select pmt_id from "
				+ this.getTableName("pmt_member_lv") + " where lv_id =? )"
				+ " and pmt_type='0' ";

		// 如果会员使作了优惠卷，将优惠卷对应的促销规则加入
		Coupons coupon = (Coupons) ThreadContextHolder.getSessionContext()
				.getAttribute("coupon");
		if (coupon != null) {
		...
		return this.daoSupport.queryForList(sql, Promotion.class, goodsid,
				memberLvId);
	}
	...
	public List<Promotion> list(Double orderPrice, Integer memberLvId) {
		...
		String sql = "select * from  " + this.getTableName("promotion")
				+ " where pmt_basic_type='order' " + " and pmt_time_begin<"
				+ now + " and  pmt_time_end>" + now
		...
		if (coupon != null) {
			int coup_pmtid = coupon.getPmt_id();
			sql += " or ( pmt_id=" + coup_pmtid
					+ " and pmt_basic_type='order')";
		}

		return this.daoSupport.queryForList(sql, Promotion.class, orderPrice,
				orderPrice, memberLvId);
	}

	private Integer getCoupPmtId(String coupcode) {
		String sql = "select  c.pmt_id from  "
				+ this.getTableName("member_coupon") + " mc ,"
				+ this.getTableName("coupons")
				+ " c where mc.cpns_id = c.cpns_id and mc.memc_code=? ";
		return this.daoSupport.queryForInt(sql, coupcode);
	}

	public void applyGoodsPmt(List<CartItem> list, Integer memberLvId) {

		if (list == null || memberLvId == null)
			return;
		...
		for (CartItem item : list) {
			...
			for (Promotion pmt : pmtList) {
				...
				if (plugin == null) {
					...
					throw new ObjectNotFoundException("plugin[" + pluginBeanId
							+ "] not found ");
				}
				...
				if (promotionMethod instanceof IDiscountBehavior) {
					...
					Double newPrice = discountBehavior.discount(pmt,
							item.getCoupPrice());
	...
	public DiscountPrice applyOrderPmt(Double orderPrice, Double shipFee,
			Integer point, Integer memberLvId) {
	...
	public void applyOrderPmt(Integer orderId, Double orderPrice,
			Integer memberLvId) {
		List<Promotion> pmtList = this.list(orderPrice, memberLvId);
		for (Promotion pmt : pmtList) {
	...
	public List listByActivityId(Integer activityid) {
		...
		return this.baseDaoSupport.queryForList(sql, Promotion.class,
				activityid);
	}
	...
	private void pluginSave(Promotion promotion) {
		...
		if (plugin == null) {
			...
			throw new ObjectNotFoundException("plugin["
					+ promotion.getPmts_id() + "] not found ");
		}
		...
		String solution = promotionMethod
				.onPromotionSave(promotion.getPmt_id());
		this.baseDaoSupport.execute(
				"update promotion set pmt_solution =? where pmt_id=?",
				solution, promotion.getPmt_id());
	...
	private void saveMemberLv(Integer pmtid, Integer[] memberLvIdArray) {
		...
		for (Integer memberLvId : memberLvIdArray) {
			...
			this.baseDaoSupport.execute(
					"insert into pmt_member_lv(pmt_id,lv_id)values(?,?)",
					pmtid, memberLvId);
		}
	}
	...
	public void saveGoods(Integer pmtid, Integer[] goodsIdArray) {
		...
		for (Integer goodsid : goodsIdArray) {
			this.baseDaoSupport.execute(
					"insert into pmt_goods(pmt_id,goods_id)values(?,?)", pmtid,
					goodsid);
		}
	}
	...
	public void delete(Integer[] pmtidArray) {
		if (pmtidArray == null || pmtidArray.length == 0)
			return;
	...
	public Promotion get(Integer pmtid) {
		return (Promotion) this.baseDaoSupport.queryForObject(
				"select * from promotion where pmt_id =?", Promotion.class,
				pmtid);
	}

	public List listGoodsId(Integer pmtid) {
		String sql = "select * from  " + this.getTableName("goods")
				+ " where goods_id in(select goods_id from "
				+ this.getTableName("pmt_goods") + " where pmt_id =? )";
	...
	public List listMemberLvId(Integer pmtid) {
		...
		return this.baseDaoSupport
				.queryForList(sql, new IntegerMapper(), pmtid);
	}

	public void useCoupon(String code, Integer memberId) {
		String sql = "select  c.* from  "
				+ this.getTableName("member_coupon")
				+ " mc ,"
				+ this.getTableName("coupons")
				+ " c where mc.cpns_id = c.cpns_id and mc.memc_code=? and mc.member_id=?";
		Coupons coupons = (Coupons) this.daoSupport.queryForObject(sql,
				Coupons.class, code, memberId);
	...
	public void setPromotionPluginBundle(
			PromotionPluginBundle promotionPluginBundle) {
