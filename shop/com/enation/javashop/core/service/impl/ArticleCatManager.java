 ...
 * 
 */
public class ArticleCatManager extends BaseSupport implements
		IArticleCatManager {
	...
	public ArticleCat getById(int cat_id) {
		...
		return (ArticleCat) this.baseDaoSupport.queryForObject(
				"select * from article_cat where cat_id=?", ArticleCat.class,
				cat_id);
	...
	public void saveAdd(ArticleCat cat) {
		...
		if (cat.getParent_id() == null)
			...
			if (cat.getCat_id() != null
					&& cat.getParent_id() == cat.getCat_id())
		...
		// 判断文章分类不能同名
		if (cat.getName() != null) {
			String sql = "select count(0) from article_cat where name = '"
					+ cat.getName() + "' and parent_id=" + cat.getParent_id();
		...
		if (cat.getParent_id() != null && cat.getParent_id().intValue() != 0) {
			...
			ArticleCat parent = (ArticleCat) this.baseDaoSupport
					.queryForObject(sql, ArticleCat.class, cat.getParent_id());
			if (parent != null) {
	...
	public void update(ArticleCat cat) {
		...
		// 判断文章分类不能同名
		if (cat.getName() != null) {
			String sql = "select count(0) from article_cat where cat_id != "
					+ cat.getCat_id() + " and name = '" + cat.getName()
					+ "' and parent_id=" + cat.getParent_id();
			int count = this.baseDaoSupport.queryForInt(sql);
			if (count > 0)
				throw new ArticleCatRuntimeException(1);// 文章分类不能同名
		}
		...
		this.baseDaoSupport.update("article_cat", map,
	...
	public void saveSort(int[] cat_ids, int[] cat_sorts) {
		String sql = "";
		if (cat_ids != null && cat_sorts != null
				&& cat_ids.length == cat_sorts.length) {
	...
	public List listChildById(Integer cat_id) {
		...
		List<ArticleCat> allCatList = this.baseDaoSupport.queryForList(sql,
				ArticleCat.class);
		...
		for (ArticleCat cat : allCatList) {
			if (cat.getParent_id().compareTo(cat_id) == 0) {
				if (this.logger.isDebugEnabled()) {
					this.logger.debug("发现子[" + cat.getName() + "-"
							+ cat.getCat_id() + "]");
				}
				List<ArticleCat> children = this.getChildren(allCatList,
						cat.getCat_id());
	...
	private List<ArticleCat> getChildren(List<ArticleCat> catList,
			Integer parentid) {
		if (this.logger.isDebugEnabled()) {
		...
		for (ArticleCat cat : catList) {
			if (cat.getParent_id().compareTo(parentid) == 0) {
				if (this.logger.isDebugEnabled()) {
					this.logger.debug(cat.getName() + "-" + cat.getCat_id()
							+ "是子");
				}
	...
	public List listHelp(int cat_id) {
		...
		String sql = "select cat_id, name  from  article_cat c  where c.parent_id = "
				+ cat_id;

		List<Map> list = this.baseDaoSupport.queryForList(sql);
		for (Map map : list) {
			List<Article> articleList = articleManager.listByCatId(Integer
					.valueOf(map.get("cat_id").toString()));
