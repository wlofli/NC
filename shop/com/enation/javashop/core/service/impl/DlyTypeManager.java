...
public class DlyTypeManager extends BaseSupport<DlyType> implements
	...
	public DlyType getDlyTypeById(Integer typeId) {
		...
		DlyType dlyType = this.baseDaoSupport.queryForObject(sql,
	...
	private DlyType fillType(DlyType type, DlyTypeConfig config) {
		...
		// 组合公式
		String expressions = this.createExpression(firstprice, continueprice,
	...
	private void addTypeArea(Integer typeId, TypeAreaConfig[] configArray) {
		for (TypeAreaConfig areaConfig : configArray) {
			...
			for (String id : idArray) {
				if (!checkedAreaId.equals(""))
					checkedAreaId += ",";
				String[] idarray = id.split("\\|");
				if (idarray.length > 1) {
					if (!closeAreaId.equals(""))
						closeAreaId += ",";
			...
			// 查询所有选中并关闭的节点的子工节点id
			List<Integer> areaIdList = this.regionsManager
					.listChildren(closeAreaId);
			// 拼入选中节点
	...
	public Double[] countPrice(Integer typeId, Double weight,
			Double orderPrice, String regionId, boolean isProtected) {
		...
		if (isProtected) {
			...
			protectPrice = dlyType.getMin_price().floatValue() > (protectPrice) ? dlyType
					.getMin_price() : protectPrice;
		}
