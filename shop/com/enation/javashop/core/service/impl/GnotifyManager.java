...
public class GnotifyManager extends BaseSupport implements IGnotifyManager {
	...
	public Page pageGnotify(int pageNo, int pageSize) {
		...
		String sql = "select a.*, b.image_default image,b.store store, b.name name, b.price price, b.mktprice mktprice,b.cat_id cat_id from "
				+ this.getTableName("gnotify")
				+ " a left join "
				+ this.getTableName("goods") + " b on b.goods_id = a.goods_id";
	...
	public void deleteGnotify(int gnotify_id) {
		this.baseDaoSupport.execute("delete from gnotify where gnotify_id = ?",
				gnotify_id);
	}
