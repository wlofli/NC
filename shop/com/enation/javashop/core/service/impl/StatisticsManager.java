 ...
 *         version 1.0
 */
public class StatisticsManager extends BaseSupport<Order> implements
		IStatisticsManager {
	...
	public List<MonthAmount> statisticsMonth_Amount() {
		...
		if (EopSetting.DBTYPE.equals("1")) {// 是mysql
			sql = "select sum(a.order_amount) as amount, Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m') as mo from "
					+ this.getTableName("order")
					+ " a where Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y') = ?  group by mo";
		} else {
			sql = "select sum(a.order_amount) as amount, SUBSTR(FROM_UNIXTIME(a.create_time / 1000),1,7) as mo from "
					+ this.getTableName("order")
					+ " a where SUBSTR(FROM_UNIXTIME(a.create_time / 1000),1,4) = ?  group by SUBSTR(FROM_UNIXTIME(a.create_time / 1000),1,7)";
		}
		...
		for (String month : monthList) {
			...
			for (Map mapdata : list) {
				if (mapdata.get("mo").equals(month)) {
					ma.setAmount(Double.valueOf(mapdata.get("amount")
							.toString()));
				}
			}
			target.add(ma);
	...
	public List<DayAmount> statisticsDay_Amount() {
		...
		String sql = "";
		if (EopSetting.DBTYPE.equals("1")) {// 是mysql
			sql = "select sum(a.order_amount) as amount, Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m-%d') as mo from "
					+ this.getTableName("order")
					+ " a where a.status = 3 and Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m') = ?  group by mo";
		} else {
			sql = "select sum(a.order_amount) as amount, SUBSTR(FROM_UNIXTIME(a.create_time / 1000),1,10) as mo from "
					+ this.getTableName("order")
					+ " a where a.status = 3 and SUBSTR(FROM_UNIXTIME(a.create_time / 1000),1,7) = ?  group by SUBSTR(FROM_UNIXTIME(a.create_time / 1000),1,10)";
		}
		List<Map> list = this.daoSupport.queryForList(sql, year);
		...
		for (String day : dayList) {
			...
			for (Map mapdata : list) {
				if (mapdata.get("mo").equals(day)) {
					da.setAmount(Double.valueOf(mapdata.get("amount")
							.toString()));
				}
			}
			target.add(da);
	...
	private static List<String> getDayList(String monthinput) {
		...
		Date date = DateUtil.toDate(monthinput + "-01", "yyyy-MM-dd");// new
																		// Date(monthinput
																		// +
																		// "-01");
