...
public class MessageManager extends BaseSupport<Message> implements
		IMessageManager {
	...
	public void delinbox(String ids) {
		this.baseDaoSupport.execute("delete from message where msg_id in ("
				+ ids + ") and del_status = '2'");
		this.baseDaoSupport
				.execute("update message set del_status = '1' where msg_id in ("
						+ ids + ")");
	}

	public void deloutbox(String ids) {
		this.baseDaoSupport.execute("delete from message where msg_id in ("
				+ ids + ") and del_status = '1'");
		this.baseDaoSupport
				.execute("update message set del_status = '2' where msg_id in ("
						+ ids + ")");
	}

	public void setMessage_read(int msg_id) {
		this.daoSupport.execute("update " + this.getTableName("message")
				+ " set unread = '1' where msg_id = ?", msg_id);
	}
