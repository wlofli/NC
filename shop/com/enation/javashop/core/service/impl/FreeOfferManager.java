 ...
 *         version 1.0<br/>
 * @since v2.1.2:将赠品图片存储改为相对，即fs:开头。读取后加上静态资源服务器地址。 <br/>
 */
public class FreeOfferManager extends BaseSupport<FreeOffer> implements
	...
	public FreeOffer get(int fo_id) {
		...
		FreeOffer freeOffer = baseDaoSupport.queryForObject(sql,
				FreeOffer.class, fo_id);
	...
	public Page list(int page, int pageSize) {
		...
		String sql = "select f.*,'' cat_name from freeoffer f where disabled=0 and publishable=0 and startdate<="
				+ now + " and enddate>=" + now + "  order by sorder asc ";
	...
	public void update(FreeOffer freeOffer) {
		this.baseDaoSupport.update("freeoffer", freeOffer,
				"fo_id=" + freeOffer.getFo_id());
	}

	private String getListSql() {
		String sql = "select fo.*,c.cat_name as cat_name from "
				+ this.getTableName("freeoffer") + " fo left join "
				+ this.getTableName("freeoffer_category")
	...
	public Page pageTrash(String name, String order, int page, int pageSize) {
		...
		name = name == null ? " fo.disabled=1 "
				: " fo.disabled=1 and fo_name like '%" + name + "%'";
		sql += " and " + name;
		sql += " order by  " + order;
	...
	public Page list(String name, String order, int page, int pageSize) {
		order = order == null ? " fo_id desc" : order;
		String sql = getListSql();
		name = name == null ? " fo.disabled=0 "
				: " fodisabled=0 and fo_name like '%" + name + "%'";
		sql += " and " + name;
		sql += " order by  " + order;
		Page webpage = this.daoSupport.queryForPage(sql, page, pageSize,
				new GiftMapper());
	...
	public List list(Integer[] ids) {
		...
		return this.baseDaoSupport.queryForList(
				"select * from freeoffer where fo_id in("
