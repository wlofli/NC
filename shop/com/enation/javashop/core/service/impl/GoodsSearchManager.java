...
public class GoodsSearchManager extends BaseSupport implements
	...
	public Page search(int page, int pageSize, Map<String, String> params) {
		...
		List list = this
				.listByCatId(typeid, cat_path, page, pageSize, order, brandStr,
						propStr, keyword, minPrice, maxPrice, tagids, attrStr);
		long count = this.countByCatId(typeid, cat_path, brandStr, propStr,
				keyword, minPrice, maxPrice, tagids, attrStr);
	...
	public List[] getPropListByCat(final int type_id, String cat_path,
		...
		temp_prop_list = temp_prop_list == null ? new ArrayList<Attribute>()
				: temp_prop_list;
		...
		if (propStr != null && !propStr.equals("")) {
			...
			for (int i = 0; i < s_ar.length; i++) {
				...
				attr.getOptionMap()[Integer.valueOf(value[1]).intValue()].put(
						"selected", 1);
		...
		temp_brand_list = this.goodsTypeManager.getBrandListByTypeId(type_id);// 这个类型关联的品牌
		...
		String sql = "select g.* from "
				...
				+ " g where g.disabled=0 and g.market_enable=1 and g.cat_id in(";
	 ...
	 */
	private List listByCatId(int typeid, String cat_path, int page,
			int pageSize, String order, String brand_str, String propStr,
		...
		RowMapper mapper = new RowMapper() {
			...
			public Object mapRow(ResultSet rs, int arg1) throws SQLException {
				...
				// 如果商品没有规格，查找唯一的货品id
				if (goods.getHasSpec() == 0 && specList != null
						&& !specList.isEmpty())
					goods.setProductid((Integer) ((Map) specList.get(0))
							.get("product_id"));
				// 计算会员价格
				if (member != null && goods.getProductid() != null) {
					goods.setPrice(goods.getPrice() * final_discount);
					for (GoodsLvPrice lvPrice : priceList) {
						if (goods.getProductid().intValue() == lvPrice
								.getProductid()) {
	 ...
	 * 应用会员价
	 */
	private void applyMemPrice(List<Map> proList,
			List<GoodsLvPrice> memPriceList, double discount) {
		for (Map pro : proList) {
			double price = ((BigDecimal) pro.get("price")).doubleValue()
					* discount;
			for (GoodsLvPrice lvPrice : memPriceList) {
				if (((Integer) pro.get("product_id")).intValue() == lvPrice
						.getProductid()) {
	...
	private long countByCatId(int typeid, String cat_path, String brand_str,
			String propStr, String keyword, String minPrice, String maxPrice,
			String tagids, String attrStr) {
		...
		sql += this.buildTermForByCat(typeid, brand_str, propStr, keyword,
	...
	private String buildTermForByCat(int typeid, String brand_str,
			String propStr, String keyword, String minPrice, String maxPrice,
			String tagids, String attrStr) {
		StringBuffer sql = new StringBuffer(buildTerm(typeid, brand_str,
				propStr, keyword, minPrice, maxPrice, attrStr));
		if (tagids != null) {
	...
	private String goodsIdInTags(String tags) {
		String sql = "select rel_id from tag_rel where tag_id in (" + tags
				+ ")";
		List<String> goodsIdList = this.baseDaoSupport.queryForList(sql,
				new StringMapper());
	...
	private String buildTermForByCat(int typeid, String brand_str,
		...
		// 属性值示例: 0_1,0_2
		if (propStr != null && !propStr.equals("")) {
			List<Attribute> prop_list = this.goodsTypeManager
					.getAttrListByTypeId(typeid); // 这个类型的属性
			prop_list = prop_list == null ? new ArrayList<Attribute>()
					: prop_list;
			String[] s_ar = propStr.split(",");
