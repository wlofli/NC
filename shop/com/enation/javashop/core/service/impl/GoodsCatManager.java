...
public class GoodsCatManager extends BaseSupport<Cat> implements
		IGoodsCatManager {
	...
	public List<Cat> listAllChildren(Integer catId) {
		...
		String sql = "select c.*,t.name as type_name  from  " + tableName
				+ " c  left join " + this.getTableName("goods_type")
				+ " t on c.type_id = t.type_id  "
			...
			if (cat.getParent_id().compareTo(catId) == 0) {
				if (this.logger.isDebugEnabled()) {
					this.logger.debug("发现子[" + cat.getName() + "-"
							+ cat.getCat_id() + "]" + cat.getImage());
				}
				List<Cat> children = this.getChildren(allCatList,
						cat.getCat_id());
	...
	public void saveAdd(Cat cat) {
		...
		if (cat.getParent_id() != null && cat.getParent_id().intValue() != 0) { // 不是顶级类别，有父
			...
			Cat parent = baseDaoSupport.queryForObject(sql, Cat.class,
					cat.getParent_id());
	...
	public void update(Cat cat) {
		...
		map.put("image",
				StringUtil.isEmpty(cat.getImage().trim()) ? null : cat
						.getImage());
