...
public class GoodsDataBatchManager implements IGoodsDataBatchManager {
	...
	private void processSheet(String datafolder, Workbook goodsWb, Node catNode) {
		...
		// 找到sheet节点
		Element beforeSheetNode = XMLUtil.getChildByTagName(catNode,
				"beforesheet");

		// 此类别数据的sheet位置
		int sheetIndex = Integer.valueOf(XMLUtil.getChildByTagName(catNode,
				"sheet_index").getTextContent());

		// 数据记录开始行位置
		int rowStartNum = Integer.valueOf(XMLUtil.getChildByTagName(catNode,
				"start_rouwnum").getTextContent());

		// 此类别对应的类别id
		int catid = Integer.valueOf(XMLUtil.getChildByTagName(catNode, "id")
				.getTextContent());

		// 商品编号所在列位置
		int goodsIdCluNum = Integer.valueOf(XMLUtil.getChildByTagName(catNode,
				"goodsid_column").getTextContent());
		...
		Element processorNode = XMLUtil
				.getChildByTagName(catNode, "processors"); // 处理器
		NodeList importerNodeList = processorNode
				.getElementsByTagName("importer");
		...
		for (int i = rowStartNum; i < lastRowNum + 1; i++) {
			...
			if (goodsIdCluNum == -1) {
				...
				goodsNum = Double.valueOf(
						row.getCell(goodsIdCluNum).getNumericCellValue())
						.intValue();
			}

			if (goodsNum != 0) {// 如果是一个新商品，生成一个新goods

				importDs.setDatafolder(datafolder
						+ "/"
						+ XMLUtil.getChildByTagName(catNode, "name")
								.getTextContent() + "/" + goodsNum);
				...
				String sn = "G"
						+ DateUtil.toString(
								...
								"yyyyMMddhhmmss") + StringUtil.getRandStr(4);
				goods.put("sn", sn);
			} else {
			...
			for (int j = 0; j < rowList.getLength(); j++) {
				...
				Object value = this.getCellValue(row.getCell(Integer
						.valueOf(index)));
				IGoodsDataImporter goodsDataImporter = SpringContextHolder
						.getBean(importer);
			 ...
			 * 执行后期处理
			 */
			Element afterSheetNode = XMLUtil.getChildByTagName(catNode,
					"aftersheet");
			...
			if (afterSheetNode != null) {
				...
				// 此类别节点的行配置列表
				NodeList afterRowList = afterSheetNode
