...
public class GoodsDescReader {
	...
	public String read(String folder, Integer goodsid) {
		...
		if (bodys != null && !bodys.isEmpty()) {
			...
			bodyHtml = bodyHtml.replaceAll("src=\"desc.files/",
					"src=\"fs:/attachment/ckeditor/" + goodsid + "/");
			// this.daoSupport.execute("update es_goods set intro=? where goods_id=?",
			// bodyHtml,goodsid);
		...
		if (new File(folderPath).exists()) {
			FileUtil.copyFolder(folderPath, EopSetting.IMG_SERVER_PATH
					+ "/attachment/ckeditor/" + goodsid);
			if (this.logger.isDebugEnabled()) {
