...
public class GoodsImageReader {
	...
	public String[] read(String folder, Integer goodsid, String excludeStr) {
		...
		for (String imgname : imgNames) {
			...
			FileUtil.copyFile(sourcePath, EopSetting.IMG_SERVER_PATH
					+ "/attachment/goods/" + goodsid + "/" + imgname);
			...
			createThumb(sourcePath, targetPath, thumbnail_pic_width,
					thumbnail_pic_height);

			targetPath = getThumbPath(imgname, "_small", goodsid);
			createThumb(sourcePath, targetPath, detail_pic_width,
					detail_pic_height);

			targetPath = getThumbPath(imgname, "_big", goodsid);
			createThumb(sourcePath, targetPath, album_pic_width,
					album_pic_height);
		...
		if (defaultImg.equals("") && imgsString != null)
			defaultImg = imgsString.split(",")[0];

		if (this.logger.isDebugEnabled()) {
			logger.debug("缩略图生成完毕，图片字串信息为image_file[" + imgsString
					+ "],defaultImg[" + defaultImg + "]");
		}
		// this.daoSupport.execute("update es_goods set image_file=? ,image_default=? where goods_id=?",
		// imgsString,defaultImg,goodsid);
	...
	private void createThumb(String sourcePath, String targetPath, int width,
			int height) {
		IThumbnailCreator thumbnailCreator = ThumbnailCreatorFactory
				.getCreator(sourcePath, targetPath);
	...
	private String getThumbPath(String filename, String shortName, int goodsid) {
		...
		String path = EopSetting.IMG_SERVER_PATH + "/attachment/goods/"
				+ goodsid;
