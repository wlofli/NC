 ...
 * 货品管理
 * 
 * @author kingapex 2010-3-9下午06:27:48
 */
public class ProductManager extends BaseSupport<Product> implements
		IProductManager {
	...
	public void add(List<Product> productList) {
		if (productList.size() > 0) {
			...
			this.baseDaoSupport.execute(
					"delete from  goods_lv_price  where goodsid=?", goodsid);

			// 清除删除的规格数据
			String sql = "delete from product where goods_id=? and  product_id  not in("
					+ this.getProductidStr(productList) + ")";
		...
		for (Product product : productList) {
			...
			if (product_id == null) {
				...
				this.baseDaoSupport.update("product", product, "product_id="
						+ product_id);
			}
			...
			for (SpecValue specvalue : specList) {
				this.daoSupport
						.execute(
								"insert into "
										+ this.getTableName("goods_spec")
										+ "(spec_id,spec_value_id,goods_id,product_id)values(?,?,?,?)",
								specvalue.getSpec_id(),
								specvalue.getSpec_value_id(),
								product.getGoods_id(), product_id);
			}
		...
		if (productList.size() > 0) {
			...
			// 更新商品的specs字段
			this.baseDaoSupport.execute(
					"update goods set specs=? where goods_id=?", JSONArray
							.fromObject(productList).toString(), goodsid);
		}
	}
	...
	public Product get(Integer productid) {
		...
		return this.baseDaoSupport
				.queryForObject(sql, Product.class, productid);
	}
	...
	public List<Specification> listSpecs(Integer goodsId) {
		...
		for (Map map : list) {
			...
			value.setSpec_value_id(Integer.valueOf(map.get("spec_value_id")
					.toString()));
	...
	public List<Product> list(Integer goodsId) {
		...
		List<Product> prolist = baseDaoSupport.queryForList(sql, Product.class,
				goodsId);

		sql = "select sv.*,gs.product_id product_id from  "
				+ this.getTableName("goods_spec")
				+ "  gs inner join "
				+ this.getTableName("spec_values")
				+ "  sv on gs.spec_value_id = sv.spec_value_id where  gs.goods_id=?";

		// sql="select * from "+
		// this.getTableName("goods_spec")+" gs,"+this.getTableName("spec_values")+" sv where gs.spec_value_id = sv.spec_value_id and  gs.goods_id=? ";
		...
		for (Product pro : prolist) {
			...
			if (member != null) {
				...
				if (memPriceList != null && memPriceList.size() > 0)
					price = this.getMemberPrice(pro.getProduct_id(),
							member.getLv_id(), price, memPriceList, discount);
				pro.setPrice(price);
			}
	 ...
	 * @return
	 */
	private Double getMemberPrice(int productid, int lvid, Double price,
		...
		for (GoodsLvPrice lvPrice : memPriceList) {
			if (lvPrice.getProductid() == productid
					&& lvPrice.getLvid() == lvid) { // 找到此货品,此会员级别的价格
	...
	public Page list(String name, String sn, int pageNo, int pageSize,
			String order) {
		order = order == null ? "product_id asc" : order;
		StringBuffer sql = new StringBuffer();
		sql.append("select p.* from " + this.getTableName("product")
				+ " p left join " + this.getTableName("goods")
				+ " g on g.goods_id = p.goods_id ");
	...
	public Product getByGoodsId(Integer goodsid) {
		...
		String sql = "select * from product where goods_id=?";
		List<Product> proList = this.baseDaoSupport.queryForList(sql,
				Product.class, goodsid);
