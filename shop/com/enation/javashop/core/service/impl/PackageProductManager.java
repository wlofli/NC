...
public class PackageProductManager extends BaseSupport implements
	...
	public void add(Goods goods, int[] product_id, int[] pkgnum) {
		String sn = "P"
				+ DateUtil.toString(new Date(System.currentTimeMillis()),
						"yyyyMMddhhmmss");
		goods.setSn(sn);
		goods.setPoint(0);
	...
	public void edit(Goods goods, int[] product_id, int[] pkgnum) {
		this.baseDaoSupport.update("goods", goods,
				"goods_id=" + goods.getGoods_id());
		this.baseDaoSupport.execute(
				"delete from package_product where goods_id = ?",
				goods.getGoods_id());
		for (int i = 0; i < product_id.length; i++) {
			PackageProduct product = new PackageProduct();
	...
	public List list(int goods_id) {
		String sql = "select pp.*, p.product_id, p.sn, p.price, p.goods_id as pgoods_id, p.weight, g.name from "
				+ this.getTableName("package_product")
				+ " pp left join "
				+ this.getTableName("product")
				+ " p on p.product_id = pp.product_id left join "
				+ this.getTableName("goods") + " g on g.goods_id = p.goods_id";
