...
public class GoodsComplexManager extends BaseSupport implements
	...
	public List listAllComplex(int goods_id) {
		// String sql =
		// "select r.*, g.goods_id, g.sn, g.name, g.image_default image, g.price, g.mktprice from "
		// + this.getTableName("goods_complex") + " r, " +
		// this.getTableName("goods") +
		// " g  where ((goods_2 = goods_id AND goods_1="+goods_id+") or (goods_1 = goods_id and goods_2 ="+goods_id+" and manual='both')) and rate > 99";

		// 查询单项关联的商品(goods1=goods_id) 或者是双向关联的商品(goods2=goods_id and
		// manual='both')
		String sql = "select   g.goods_id, g.sn, g.name, g.image_default image, g.price, g.mktprice from "
				+ this.getTableName("goods")
				+ " g where g.goods_id in (select goods_2 from "
				+ this.getTableName("goods_complex")
				+ " where goods_1="
				+ goods_id
				+ " )"
				+ " or goods_id in( select goods_1 from "
				+ this.getTableName("goods_complex")
				+ " where goods_2="
				+ goods_id + " and manual='both'  )";
	...
	public List listComplex(int goods_id) {
		...
		String sql = "select * from " + this.getTableName("goods") + " g, "
				+ this.getTableName("goods_complex")
				+ " c where goods_2 =goods_id and  goods_1=?";
