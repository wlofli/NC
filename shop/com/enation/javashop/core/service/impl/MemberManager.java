 ...
 */
public class MemberManager extends BaseSupport<Member> implements
		IMemberManager {
	...
	public void logout() {
		...
		ThreadContextHolder.getSessionContext().removeAttribute(
				IUserService.CURRENT_MEMBER_KEY);
	...
	public Member get(Integer memberId) {
		...
		Member m = this.baseDaoSupport.queryForObject(sql, Member.class,
				memberId);
		return m;
	}
	...
	public Member edit(Member member) {
		...
		this.baseDaoSupport.update("member", member,
				"member_id=" + member.getMember_id());
		ThreadContextHolder.getSessionContext().setAttribute(
				IUserService.CURRENT_MEMBER_KEY, member);
		return null;
	}
	...
	public Page list(String order, int page, int pageSize) {
		...
		String sql = "select m.*,lv.name as lv_name from "
				...
				+ this.getTableName("member_lv") + " lv on m.lv_id = lv.lv_id ";
		// sql +=
		// "  and lv.userid = "+this.getCurrentUserid()+" and lv.siteid="+this.getCurrentSiteid();
		sql += " order by  " + order;
	...
	public int login(String username, String password) {
		...
		List<Member> list = this.daoSupport.queryForList(sql, Member.class,
				username, com.enation.framework.util.StringUtil.md5(password));
		...
		if (DateUtil.toString(date, "yyyy-MM").equals(
	...
	public Page list(String order, String name, String uname, int page,
			int pageSize) {
		order = order == null ? " m.member_id desc" : order;
		String sql = "select m.*,lv.name as lv_name from "
				+ this.getTableName("member") + " m left join "
	...
	public static void main(String[] args) {
		System.out.println(DateUtil.toDate("2011-05-27", "yyyy-MM-dd")
				.getTime());
	}
