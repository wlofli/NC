 ...
 * 
 */
public class MemberPointManger extends BaseSupport implements
		IMemberPointManger {
	...
	public void useMarketPoint(int memberid, int point, String reson,
			Integer relatedId) {
		...
		this.baseDaoSupport.execute(
				"update member set mp=mp-? where member_id=?", point, memberid);
	}
	...
	public void add(int memberid, int point, String itemname, Integer relatedId) {
		...
		this.baseDaoSupport.execute(
				"update member set point=point+? where member_id=?", point,
				memberid);
		...
		if (memberpoint != null) {
			...
			if (lv != null) {
				if (member.getLv_id() == null
						|| lv.getLv_id().intValue() != member.getLv_id()
								.intValue()) {
