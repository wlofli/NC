 ...
 * 订单业务流程
 * 
 * @author kingapex 2010-4-8上午10:19:42
 */
public class OrderFlowManager extends BaseSupport implements IOrderFlowManager {
	...
	public void pay(PaymentLog payment, boolean isOnline) {
		...
		if (order.getMember_id() != null) {
			...
			if (memberPointManger.checkIsOpen(IMemberPointManger.TYPE_BUYGOODS)) {
				int point = memberPointManger
						.getItemPoint(IMemberPointManger.TYPE_BUYGOODS + "_num");
				point = order.getGoods_amount().intValue() * point;
				memberPointManger.add(order.getMember_id(), point,
						IMemberPointManger.TYPE_BUYGOODS, order.getOrder_id());
			...
			if (isOnline
					&& memberPointManger
							.checkIsOpen(IMemberPointManger.TYPE_ONLINEPAY)) {
				int point = memberPointManger
						.getItemPoint(IMemberPointManger.TYPE_ONLINEPAY
								+ "_num");
				memberPointManger.add(order.getMember_id(), point,
						IMemberPointManger.TYPE_ONLINEPAY, order.getOrder_id());
			}
		...
		this.baseDaoSupport
				.execute(
						"update order set status=?,pay_status=?,paymoney=paymoney+? where order_id=?",
						OrderStatus.ORDER_PAY, payStatus, payment.getMoney(),
						payment.getOrder_id());

		this.log(payment.getOrder_id(), "支付订单，金额" + payment.getMoney(), null,
				"管理员");
	...
	private void savePoint(Integer point, Integer memberid) {
		...
		Member sessionMember = UserServiceFactory.getUserService()
				.getCurrentMember();
	...
	private void saveMoney(Order order, PaymentLog payment) {
		if (logger.isDebugEnabled()) {
			logger.debug("扣除用户[" + order.getMember_id() + "]预存款:"
					+ payment.getMoney());
		}
		Member member = this.memberManager.get(order.getMember_id());
		if (member.getAdvance().compareTo(payment.getMoney()) < 0) {
			if (logger.isDebugEnabled()) {
				logger.debug("用户预存款[" + member.getAdvance().doubleValue()
						+ "]不足,需要支付[" + payment.getMoney().doubleValue() + "]");
			}
			throw new RuntimeException("用户预存款["
					+ member.getAdvance().doubleValue() + "]不足,需要支付["
					+ payment.getMoney().doubleValue() + "]");
		}
		baseDaoSupport.execute(
				"update member set advance=advance-? where member_id=?",
				payment.getMoney(), order.getMember_id());
		...
		if (sessionMember != null) {
			...
			sessionMember.setAdvance(CurrencyUtil.sub(
					sessionMember.getAdvance(), payment.getMoney()));
		}
	}
	...
	public void refund(PaymentLog payment) {
		...
		if (order.getPay_status().intValue() == OrderStatus.PAY_CANCEL) {
			...
			throw new IllegalStateException("订单[" + order.getSn()
					+ "]支付状态为[已经退款]，不能再对其进行退款操作");
		}
		...
		if (logger.isDebugEnabled()) {
			logger.debug("更新订单状态[" + OrderStatus.ORDER_CANCEL_PAY + "],支付状态["
					+ payStatus + "]");
		}
		baseDaoSupport
				.execute(
						"update order set status=?,pay_status=?,paymoney=paymoney-? where order_id=?",
						OrderStatus.ORDER_CANCEL_PAY, payStatus,
						payment.getMoney(), payment.getOrder_id());
		// 返还用户预存款
		...
		if (payment.getPay_type().trim().equals("deposit")) {
			if (order.getMember_id() != null) {
				if (logger.isDebugEnabled()) {
					logger.debug("返还用户[" + order.getMember_id() + "]预存款:"
							+ payment.getMoney());
				}
				baseDaoSupport
						.execute(
								"update member set advance=advance+? where member_id=?",
								payment.getMoney(), payment.getMember_id());
				// 既然是用预存款支付，则应加到预存款日志中
		...
		this.log(payment.getOrder_id(), "订单退款，金额" + payment.getMoney(), null,
				"管理员");
	 ...
	 * @param op_name
	 */
	private void log(Integer order_id, String message, Integer op_id,
			String op_name) {
	...
	public void shipping(Delivery delivery, List<DeliveryItem> itemList,
			List<DeliveryItem> giftItemList) {
		...
		// 处理商品发货项
		int goodsShipStatus = this.processGoodsShipItem(orderId, delivery_id,
				itemList);
		shipStatus = goodsShipStatus;
		if (giftItemList != null && !giftItemList.isEmpty()) {
			// 处理赠品发货项
			int giftShipStatus = this.processGiftShipItem(orderId, delivery_id,
					giftItemList);
			shipStatus = (giftShipStatus == OrderStatus.SHIP_YES && goodsShipStatus == OrderStatus.SHIP_YES) ? OrderStatus.SHIP_YES
					: OrderStatus.SHIP_PARTIAL_SHIPED;
		}

		// 如果发货状态不为全部发货即为部分发货
		shipStatus = shipStatus == OrderStatus.SHIP_YES ? OrderStatus.SHIP_YES
				: OrderStatus.SHIP_PARTIAL_SHIPED;
		...
		if (logger.isDebugEnabled()) {
			logger.debug("更新订单[" + orderId + "]状态[" + OrderStatus.ORDER_SHIP
					+ "]，发货状态[" + shipStatus + "]");
		}
		// 更新订单状态为已发货
		baseDaoSupport.execute(
				"update order set status=?,ship_status=? where order_id=?",
				OrderStatus.ORDER_SHIP, shipStatus, orderId);
		this.log(delivery.getOrder_id(), "订单发货，物流单据号：" + delivery.getLogi_no(),
				null, "管理员");
	 ...
	 *            发货单id
	 * @param orderItemList
	 *            订单商品货物 {@link IOrderManager#listGoodsItems(Integer)}
	 * @param itemList
	 *            由用户表单读取的退货数据
	 * @return 商品退货状态
	 */
	private int processGoodsReturnItem(Integer orderId, Integer delivery_id,
		...
		for (DeliveryItem item : itemList) {
			...
			shipStatus = (shipStatus == OrderStatus.SHIP_CANCEL && itemStatus == OrderStatus.SHIP_CANCEL) ? OrderStatus.SHIP_CANCEL
					: itemStatus;
			...
			baseDaoSupport
					.execute(
							"update order_items set ship_num=ship_num-? where order_id=? and spec_id=?",
							item.getNum(), orderId, item.getProduct_id());

			// 更新库存量
			baseDaoSupport.execute(
					"update goods set store=store+? where goods_id=?",
					item.getNum(), item.getGoods_id());
			baseDaoSupport.execute(
					"update product set store=store+? where product_id=?",
					item.getNum(), item.getProduct_id());
	 ...
	 *            发货单id
	 * @param orderItemList
	 *            订单商品货物 {@link IOrderManager#listGiftItems(Integer)}
	 * @param itemList
	 *            由用户表单读取的退货数据
	 * @return 赠品退货状态
	 */
	private int processGiftReturnItem(Integer orderId, Integer delivery_id,
			List<DeliveryItem> itemList) {
		...
		for (DeliveryItem item : itemList) {
			...
			baseDaoSupport
					.execute(
							"update order_gift set shipnum=shipnum-? where order_id=? and gift_id=?",
							item.getNum(), orderId, item.getGoods_id());

			// 更新库存量
			baseDaoSupport.execute(
					"update freeoffer set repertory=repertory+? where fo_id=?",
					item.getNum(), item.getGoods_id());
	...
	private void fillAdjItem(Integer orderid, List<DeliveryItem> itemList) {
		...
		for (DeliveryItem dlyItem : itemList) {
			for (Map adjItem : adjItemList) {
				// 此货品对应的配件
				if (dlyItem.getProduct_id().intValue() == ((Integer) adjItem
						.get("spec_id")).intValue() && dlyItem.getNum() > 0) {

					String addonstr = (String) adjItem.get("addon");
					if (addonstr != null) {
						Collection<AdjunctItem> collection = JSONArray
								.toCollection(JSONArray.fromObject(addonstr),
										AdjunctItem.class);
	 ...
	 * @return
	 */
	private int processGoodsShipItem(Integer orderId, Integer delivery_id,
			List<DeliveryItem> itemList) {
		...
		for (DeliveryItem item : itemList) {
			...
			shipStatus = (shipStatus == OrderStatus.SHIP_YES && itemStatus == OrderStatus.SHIP_YES) ? OrderStatus.SHIP_YES
					: itemStatus;
			...
			baseDaoSupport.execute(
					...
					item.getNum(), item.getProduct_id());
			if (this.logger.isDebugEnabled()) {
				this.logger.debug("更新" + item.getName() + "["
						+ item.getProduct_id() + "," + item.getGoods_id()
						+ "-[" + item.getNum() + "]");
			}
		}
	 ...
	 * @return
	 */
	private int processGiftShipItem(Integer orderId, Integer delivery_id,
			List<DeliveryItem> itemList) {
	 ...
	 * 
	 * @see
	 * com.enation.javashop.core.service.IOrderFlowManager#applyReturns(java
	 * .lang.Integer, int[])
	 */
	public void applyReturns(Integer orderid, int state, Integer[] specids) {

		if (orderid == null)
			throw new IllegalArgumentException("param orderid id is null");
		if (specids == null)
			throw new IllegalArgumentException("param specids is NULL");
		...
		this.baseDaoSupport
				.execute(
						"update order_items set state=1 where order_id=? and spec_id in("
								+ StringUtil.arrayToString(specids, ",") + ")",
						orderid);
		this.baseDaoSupport.execute(
				"update order set status=? where order_id=? ", orderstate,
				orderid);
	}
	...
	public void refuseRorC(Integer orderid) {
		...
		if (order.getStatus().intValue() == OrderStatus.ORDER_CHANGE_APPLY)
			state = -6; // 如果是换货则状态 为换货被拒绝

		this.baseDaoSupport.execute(
				"update order_items set state=2 where order_id=? and state=1",
				orderid);
		this.baseDaoSupport.execute(
				"update order set status=? where order_id=? ", state, orderid);
	}
	...
	public void returned(Delivery delivery, List<DeliveryItem> itemList,
		...
		// 处理退货状态
		int shipStatus = OrderStatus.SHIP_CANCEL;
		int goodsShipStatus = this.processGoodsReturnItem(orderId, delivery_id,
				itemList);
		shipStatus = goodsShipStatus;
		if (giftItemList != null) {
			int giftShipStatus = this.processGiftReturnItem(orderId,
					delivery_id, giftItemList);
			// 如果商品货品和赠品货品都退货完成则 退货状态为退货完成，否则为部分退货
			shipStatus = (giftShipStatus == OrderStatus.SHIP_CANCEL && goodsShipStatus == OrderStatus.SHIP_CANCEL) ? OrderStatus.SHIP_CANCEL
					: OrderStatus.SHIP_PARTIAL_CANCEL;
		}
		// 如果退货完成则 退货状态为退货完成，否则为部分退货
		shipStatus = shipStatus == OrderStatus.SHIP_CANCEL ? OrderStatus.SHIP_CANCEL
				: OrderStatus.SHIP_PARTIAL_CANCEL;
		...
		if (logger.isDebugEnabled()) {
			logger.debug("更新订单[" + orderId + "]状态["
					+ OrderStatus.ORDER_CANCEL_SHIP + "]，发货状态[" + shipStatus
					+ "]");
		}
		// 更新订单状态为已发货
		baseDaoSupport.execute(
				"update order set status=?,ship_status=? where order_id=?",
				OrderStatus.ORDER_CANCEL_SHIP, shipStatus, orderId);
		this.log(delivery.getOrder_id(), "订单退货，物流单据号：" + delivery.getLogi_no(),
				null, "管理员");
	}

	public void change(Delivery delivery, List<DeliveryItem> itemList,
			List<DeliveryItem> gifitemList) {
		if (delivery == null)
			throw new IllegalArgumentException("param delivery is NULL");
		if (itemList == null)
			throw new IllegalArgumentException("param itemList is NULL");
		...
		baseDaoSupport.execute(
				"update order set status=?, ship_status=? where order_id=?",
				OrderStatus.ORDER_CHANGED, OrderStatus.SHIP_CHANED, orderId);
	 ...
	 * 
	 * @param orderItemList
	 *            购买的订单货物信息 @see {@link IOrderManager#listGoodsItems(Integer)}
	 * @param item
	 *            某一个发货项
	 * @return
	 */
	private int checkGoodsReturnNum(List<Map> orderItemList, DeliveryItem item) {
		...
		for (Map orderItem : orderItemList) {
			...
			if (Integer.valueOf(orderItem.get("spec_id").toString()).compareTo(
					item.getProduct_id()) == 0) {

				Integer shipNum = Integer.valueOf(orderItem.get("ship_num")
						.toString()); // 已经发货量
				if (shipNum < item.getNum()) { // 已发货量小于本次 退货量
					if (logger.isDebugEnabled()) {
						logger.debug("商品[" + item.getName() + "]本次发货量["
								+ item.getNum() + "]超出已发货量[" + shipNum + "]");
					}
					throw new RuntimeException("商品[" + item.getName()
							+ "]本次发货量[" + item.getNum() + "]超出已发货量[" + shipNum
							+ "]");
				}
	 ...
	 * 
	 * @param orderItemList
	 *            订单赠品货物列表 {@link IOrderManager#listGoodsItems(Integer)}
	 * @param item
	 *            由用户表单读取到的赠品退货数据
	 * @return
	 */
	private int checkGiftReturnNum(List<Map> orderItemList, DeliveryItem item) {
		...
		for (Map orderItem : orderItemList) {
			...
			if (Integer.valueOf(orderItem.get("gift_id").toString()).compareTo(
					item.getGoods_id()) == 0) {

				Integer shipNum = Integer.valueOf(orderItem.get("shipnum")
						.toString()); // 已经发货量
				if (shipNum < item.getNum()) { // 已发货量小于本次 退货量
					if (logger.isDebugEnabled()) {
						logger.debug("赠品[" + item.getName() + "]本次发货量["
								+ item.getNum() + "]超出已发货量[" + shipNum + "]");
					}
					throw new RuntimeException("赠品[" + item.getName()
							+ "]本次发货量[" + item.getNum() + "]超出已发货量[" + shipNum
							+ "]");
				}
				if (shipNum.compareTo(item.getNum()) == 0) { // 已发货量等于本次退货量，状态为已退货
	...
	public void complete(Integer orderId) {
		...
		this.baseDaoSupport.execute(
				...
				OrderStatus.ORDER_COMPLETE, orderId);
		this.baseDaoSupport.execute(
				"update order set complete_time=? where order_id=?",
				DateUtil.getDatelineLong(), orderId);
	...
	public void cancel(Integer orderId) {
		...
		this.baseDaoSupport.execute(
				"update order set status=? where order_id=?",
				OrderStatus.ORDER_CANCELLATION, orderId);
		this.log(orderId, "订单作废：", null, "管理员");
	}

	public List<OrderItem> listNotShipGoodsItem(Integer orderId) {
		String sql = "select oi.*,p.store,p.sn from "
				+ this.getTableName("order_items") + "  oi left join "
				+ this.getTableName("product")
				+ " p on oi.spec_id = p.product_id";
		...
		for (Map orderItem : orderItemList) {
			...
			if (Integer.valueOf(orderItem.get("spec_id").toString()).compareTo(
				...
				if (num < item.getNum() + shipNum) { // 总购买量小于总发货量
					if (logger.isDebugEnabled()) {
						logger.debug("商品[" + item.getName() + "]本次发货量["
								+ item.getNum() + "]后超出用户购买量[" + num
								+ "],此商品已经发货[" + shipNum + "]");
					}
					throw new RuntimeException("商品[" + item.getName()
							+ "]本次发货量[" + item.getNum() + "]后超出用户购买量[" + num
							+ "],此商品已经发货[" + shipNum + "]");
				}
	 ...
	 * 检查要发货商品的库存
	 */
	private void checkGoodsStore(Integer orderId, List<Product> productList,
			DeliveryItem item) {
		...
		for (Product product : productList) {
			if (product.getProduct_id().compareTo(item.getProduct_id()) == 0) {
				if (product.getStore().compareTo(item.getNum()) < 0) { // 库存小于发货量
					if (logger.isDebugEnabled()) {
						logger.debug("商品[" + item.getName() + "]库存["
								+ product.getStore() + "]不足->发货量["
								+ item.getNum() + "]");
					}

					throw new RuntimeException("商品[" + item.getName() + "]库存["
							+ product.getStore() + "]不足->发货量[" + item.getNum()
							+ "]");
				}
			}
		}
	 ...
	 * @param item
	 */
	private void checkGiftStore(Integer orderId, List<FreeOffer> giftList,
			DeliveryItem item) {
		...
		for (FreeOffer freeoffer : giftList) {
			if (freeoffer.getFo_id().compareTo(item.getGoods_id()) == 0) {
				if (freeoffer.getRepertory().compareTo(item.getNum()) < 0) { // 库存小于发货量
					if (logger.isDebugEnabled()) {
						logger.debug("赠品[" + item.getName() + "]库存["
								+ freeoffer.getRepertory() + "]不足->发货量["
								+ item.getNum() + "]");
					}

					throw new RuntimeException("赠品[" + item.getName() + "]库存["
							+ freeoffer.getRepertory() + "]不足->发货量["
							+ item.getNum() + "]");
				}
			}
	...
	private List<Product> listProductbyOrderId(Integer orderId) {
		...
		List<Product> productList = this.daoSupport.queryForList(sql,
	...
	private List<FreeOffer> listGiftByOrderId(Integer orderId) {
		...
		List<FreeOffer> productList = this.daoSupport.queryForList(sql,
				FreeOffer.class, orderId);
	...
	private void checkDisabled(Order order) {
		if (order.getStatus().intValue() == OrderStatus.ORDER_COMPLETE
				|| order.getStatus().intValue() == OrderStatus.ORDER_CANCELLATION)
