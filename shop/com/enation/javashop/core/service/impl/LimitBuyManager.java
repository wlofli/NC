...
public class LimitBuyManager extends BaseSupport implements ILimitBuyManager {
	...
	private void addGoods(List<LimitBuyGoods> limitBuyGoodsList,
			Integer limitBuyId) {
		if (limitBuyGoodsList == null || limitBuyGoodsList.isEmpty())
			throw new RuntimeException("添加限时购买的商品列表不能为空");
		for (LimitBuyGoods limitBuyGoods : limitBuyGoodsList) {
			this.baseDaoSupport
					.execute(
							"insert into limitbuy_goods (limitbuyid,goodsid,price)values(?,?,?)",
							limitBuyId, limitBuyGoods.getGoodsid(),
							limitBuyGoods.getPrice());
	...
	public void delete(Integer id) {
		...
		this.baseDaoSupport.execute(
				"delete from limitbuy_goods where limitbuyid=?", id);
	...
	public void edit(LimitBuy limitBuy) {
		List<LimitBuyGoods> limitBuyGoodsList = limitBuy.getLimitBuyGoodsList();
		this.baseDaoSupport.update("limitbuy", limitBuy,
				"id=" + limitBuy.getId());
		Integer limitBuyId = limitBuy.getId();
		this.baseDaoSupport.execute(
				"delete from limitbuy_goods where limitbuyid=?", limitBuyId);
	...
	private void updateGoodsLimit(Integer limitid, int islimit) {
		this.daoSupport.execute("update " + this.getTableName("goods")
				+ " set islimit=? where goods_id in(select goodsid from "
				+ this.getTableName("limitbuy_goods") + " where limitbuyid=?)",
				islimit, limitid);
	}
	...
	public List<LimitBuy> listEnable() {
		...
		List<LimitBuy> limitBuyBuyList = this.baseDaoSupport.queryForList(sql,
				LimitBuy.class, now, now);

		// 读取出相应的商品列表
		sql = "select g.* ,lg.limitbuyid limitbuyid ,lg.price limitprice  from "
				+ this.getTableName("limitbuy_goods")
				+ " lg  ,"
				+ this.getTableName("goods")
				+ " g where lg.goodsid= g.goods_id and lg.limitbuyid in";
		sql += "(select id from " + this.getTableName("limitbuy")
	...
	public List<Map> listEnableGoods() {
		long now = DateUtil.getDatelineLong();
		// 读取出相应的商品列表
		String sql = "select g.* ,lg.limitbuyid limitbuyid ,lg.price limitprice  from "
				+ this.getTableName("limitbuy_goods")
				+ " lg  ,"
				+ this.getTableName("goods")
				+ " g where lg.goodsid= g.goods_id and lg.limitbuyid in";
	...
	public LimitBuy get(Integer id) {
		...
		LimitBuy limitBuy = (LimitBuy) this.baseDaoSupport.queryForObject(sql,
				LimitBuy.class, id);
		sql = "select g.* ,lg.limitbuyid limitbuyid from "
				+ this.getTableName("limitbuy_goods") + " lg  ,"
				+ this.getTableName("goods")
				+ " g where lg.goodsid= g.goods_id and lg.limitbuyid =?";
