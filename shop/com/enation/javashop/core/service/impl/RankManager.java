...
public class RankManager extends BaseSupport implements IRankManager {
	...
	public List rank_goods(int page, int pageSize, String condition, String sort) {
		String sql = "select items.name name, sum(items.price * items.num) amount, sum(items.num) num from "
				+ this.getTableName("order")
				+ " orders , "
				+ this.getTableName("order_items")
				+ " items where items.order_id = orders.order_id";
	...
	public List rank_member(int page, int pageSize, String condition,
			String sort) {
		String sql = "select member.uname uname, member.name name, sum(items.price * items.num) amount, sum(items.num) num from "
				+ this.getTableName("order")
				+ " orders , "
				+ this.getTableName("order_items")
				+ " items, "
				+ this.getTableName("member")
				+ " member where items.order_id = orders.order_id and member.member_id = orders.member_id";
		if (condition == null || condition.equals("")) {
			condition = " and orders.status = 3";
		} else {
			condition += " and orders.status = 3";
	...
	public Map rank_all() {
		Map map = this.baseDaoSupport
				.queryForMap("select sum(order_amount) amount, count(order_id) num from order where status = 3");
		int view_count = this.baseDaoSupport
				.queryForInt("select sum(view_count) view_count from goods where disabled = 0");
		map.put("view_count", view_count);
		int member_count = this.baseDaoSupport
				.queryForInt("select count(member_id) member_count from member");
		map.put("member_count", member_count);
		int buy_member_count = this.baseDaoSupport
				.queryForInt("select count(distinct member_id) buy_member_count from order where status = 3 and member_id > 0");
