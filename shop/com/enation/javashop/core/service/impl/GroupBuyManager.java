...
public class GroupBuyManager extends BaseSupport implements IGroupBuyManager {
	...
	public void add(GroupBuy groupBuy) {
		if (groupBuy.getDis_type() == 0 && groupBuy.getDiscount() != null) {
			Double price = this.goodsManager.getGoods(groupBuy.getGoodsid())
					.getPrice();
	...
	private void updateGoodsGroupBuy(int goodsid, int isgroup) {
		this.baseDaoSupport
				.execute("update goods set isgroup=? where goods_id=?",
						isgroup, goodsid);
	}
	...
	public void delete(int groupid) {
		...
		this.baseDaoSupport.execute(
				"delete from group_buy_count where groupid=? ", groupid);
		this.baseDaoSupport.execute("delete from group_buy where groupid=? ",
				groupid);
	}
	...
	public void edit(GroupBuy groupBuy) {
		...
		this.baseDaoSupport.update("group_buy", groupBuy,
				"groupid=" + groupBuy.getGroupid());
		this.baseDaoSupport.execute(
				"delete from group_buy_count where groupid=? ",
				groupBuy.getGroupid());
		List<GroupBuyCount> list = groupBuy.getCountRuleList();
		for (GroupBuyCount buyCount : list) {
	...
	public GroupBuy get(int groupid) {
		GroupBuy groupBuy = (GroupBuy) this.baseDaoSupport.queryForObject(
				"select  * from group_buy where groupid=?", GroupBuy.class,
				groupid);
	...
	public Page list(int pageNo, int pageSize) {
		return this.baseDaoSupport.queryForPage(
				"select  * from group_buy order by add_time desc", pageNo,
				pageSize, GroupBuy.class);
	}
	...
	public List<GroupBuy> listEnable() {
		...
		return this.daoSupport
				.queryForList(
						"select gb.* ,g.cat_id catid ,g.price oldprice  from "
								+ this.getTableName("group_buy")
								+ " gb , "
								+ this.getTableName("goods")
								+ " g where gb.goodsid = g.goods_id and  gb.start_time<? and gb.end_time>? order by add_time desc",
						GroupBuy.class, now, now);
	}

	public List<GroupBuyCount> listRule(int groupid) {

		return this.baseDaoSupport.queryForList(
				"select * from group_buy_count where groupid =? ",
				GroupBuyCount.class, groupid);
	}
	...
	public void addBuyCount() {
		...
		// 读取出可用团购的数量增长规则
		List<GroupBuyCount> rulelist = this.daoSupport
				.queryForList(
						"select * from es_group_buy_count where groupid in(select groupid from es_group_buy where start_time<? and end_time>?) ",
						GroupBuyCount.class, now, now);
		...
		for (GroupBuy groupBuy : groupBuyList) {
			...
			for (GroupBuyCount buyCount : rulelist) {
				...
				if (groupBuy.getGroupid() == buyCount.getGroupid()
						&& start <= hour && end > hour) {
					int num = buyCount.getNum();
					this.baseDaoSupport
							.execute(
									"update group_buy set buy_count= buy_count+? where groupid=?",
									num, groupBuy.getGroupid());
					if (this.logger.isDebugEnabled()) {
						this.logger.debug("更新团购[" + groupBuy.getTitle()
								+ "]团购数加[" + num + "]");
					}
					break;
				}
	...
	public static void main(String[] args) {
		...
		long now = DateUtil.getDatelineLong();
		long end = (long) (DateUtil.toDate("2011-03-31 19:53",
