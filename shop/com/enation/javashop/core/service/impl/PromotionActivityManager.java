...
public class PromotionActivityManager extends BaseSupport<PromotionActivity>
	...
	public void add(PromotionActivity activity) {
		...
		if (activity.getName() == null)
			throw new IllegalArgumentException("param activity.name is NULL");
		if (activity.getBegin_time() == null)
			throw new IllegalArgumentException(
					"param activity.begin_time is NULL");
		if (activity.getEnd_time() == null)
			throw new IllegalArgumentException(
					"param activity.end_time is NULL");
	...
	public void delete(Integer[] idArray) {
		if (idArray != null && idArray.length > 0) {
			...
			this.baseDaoSupport
					.execute("delete from promotion_activity where id in ("
							+ ids + ")");

			this.daoSupport.execute(
					"delete from " + this.getTableName("pmt_member_lv")
							+ " where pmt_id in(select pmt_id from "
							+ this.getTableName("promotion")
							+ " where pmta_id in(?))", ids);
			this.daoSupport.execute(
					"delete from " + this.getTableName("pmt_goods")
							+ " where pmt_id in(select pmt_id from "
							+ this.getTableName("promotion")
							+ " where pmta_id in(?))", ids);
			this.baseDaoSupport.execute(
					"delete from promotion where pmta_id in(?)", ids);
		}
	...
	public void edit(PromotionActivity activity) {
		if (activity.getId() == null)
			throw new IllegalArgumentException("param activity.id is NULL");
		if (activity.getName() == null)
			throw new IllegalArgumentException("param activity.name is NULL");
		if (activity.getBegin_time() == null)
			throw new IllegalArgumentException(
	...
	public PromotionActivity get(Integer id) {
		...
		if (activity == null)
			throw new ObjectNotFoundException("activity is NULL");
		return activity;
	}
