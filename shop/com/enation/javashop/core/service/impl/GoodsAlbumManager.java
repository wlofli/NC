 ...
 * 商品相册管理
 * 
 * @author kingapex 2010-2-21上午12:40:05
 */
public final class GoodsAlbumManager extends BaseSupport<Goods> implements
		IGoodsAlbumManager {
	...
	public void delete(String photoName) {
		if (photoName != null
				&& !photoName
						.startsWith("http://static.enationsoft.com/attachment/")) {
			photoName = UploadUtil.replacePath(photoName);
			photoName = photoName.replaceAll(EopSetting.IMG_SERVER_DOMAIN,
	...
	public void delete(Integer[] goodsid) {
		...
		String sql = "select image_file from goods where goods_id in ("
				+ id_str + ")";
	...
	public String[] upload(File file, String fileFileName) {
		...
		if (file != null && fileFileName != null) {
			...
			filePath = EopSetting.IMG_SERVER_PATH + getContextFolder()
					+ "/attachment/goods/";
			path[0] = EopSetting.IMG_SERVER_DOMAIN + getContextFolder()
					+ "/attachment/goods/" + fileName;
			filePath += fileName;
			...
			if (watermark != null && watermark.equals("on")) {
				...
				magickMask.mask(filePath, marktext, markcolor,
						Integer.valueOf(marksize), Integer.valueOf(markpos),
	 ...
	 * 如果是公网上的静态资源则不处理
	 */
	public void createThumb(String filepath, String thumbName,
			int thumbnail_pic_width, int thumbnail_pic_height) {
		String imgDomain = "http://static.enationsoft.com/attachment/"; // EopSetting.IMG_SERVER_DOMAIN.startsWith("http")?
																		// EopSetting.IMG_SERVER_DOMAIN:"http://"+EopSetting.IMG_SERVER_DOMAIN;
		if (filepath != null && !filepath.startsWith(imgDomain)) {
			filepath = filepath.replaceAll(EopSetting.IMG_SERVER_DOMAIN,
					EopSetting.IMG_SERVER_PATH);
			thumbName = thumbName.replaceAll(EopSetting.IMG_SERVER_DOMAIN,
					EopSetting.IMG_SERVER_PATH);
			IThumbnailCreator thumbnailCreator = ThumbnailCreatorFactory
					.getCreator(filepath, thumbName);
	...
	public void recreate(int start, int end) {
		 ...
		 */
		try {
			thumbnail_pic_width = Integer.valueOf(getSettingValue(
					"thumbnail_pic_width").toString());
			thumbnail_pic_height = Integer.valueOf(getSettingValue(
					"thumbnail_pic_height").toString());

			detail_pic_width = Integer.valueOf(getSettingValue(
					"detail_pic_width").toString());
			detail_pic_height = Integer.valueOf(getSettingValue(
					"detail_pic_height").toString());

			album_pic_width = Integer
					.valueOf(getSettingValue("album_pic_width").toString());
			album_pic_height = Integer.valueOf(getSettingValue(
					"album_pic_height").toString());
		...
		for (int i = start - 1; i < end; i++) {
			...
			if (imageFile != null) {
				...
				System.out.println("Create thumbnail image, the index:"
						+ (i + 1));
				for (String imgFile : imgFileAr) {
					System.out.println("Image file:" + imgFile);
					// 生成缩略图
					String thumbName = UploadUtil.getThumbPath(imgFile,
							"_thumbnail");
					this.createThumb1(imgFile, thumbName, thumbnail_pic_width,
							thumbnail_pic_height);

					// 生成小图
					thumbName = UploadUtil.getThumbPath(imgFile, "_small");
					createThumb1(imgFile, thumbName, detail_pic_width,
							detail_pic_height);

					// 生成大图
					thumbName = UploadUtil.getThumbPath(imgFile, "_big");
					createThumb1(imgFile, thumbName, album_pic_width,
							album_pic_height);
	 ...
	 */
	private void createThumb1(String filepath, String thumbName,
			int thumbnail_pic_width, int thumbnail_pic_height) {
		if (!StringUtil.isEmpty(filepath)) {
			String ctx = EopContext.getContext().getContextPath();
			filepath = filepath.replaceAll(EopSetting.FILE_STORE_PREFIX,
					EopSetting.IMG_SERVER_PATH + ctx);
			thumbName = thumbName.replaceAll(EopSetting.FILE_STORE_PREFIX,
					EopSetting.IMG_SERVER_PATH + ctx);
			IThumbnailCreator thumbnailCreator = ThumbnailCreatorFactory
					.getCreator(filepath, thumbName);
			thumbnailCreator.resize(thumbnail_pic_width, thumbnail_pic_height);
