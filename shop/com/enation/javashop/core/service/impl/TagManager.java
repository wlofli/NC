 ...
 * Saas式的标签管理
 * 
 * @author kingapex 2010-1-18上午10:57:35
 */
public class TagManager extends BaseSupport<Tag> implements ITagManager {
	...
	public void delete(Integer[] tagids) {
		...
		this.baseDaoSupport.execute("delete from tags where tag_id in(" + ids
				+ ")");
		this.daoSupport.execute("delete from " + this.getTableName("tag_rel")
				+ " where tag_id in(" + ids + ")");
	}

	public Page list(int pageNo, int pageSize) {

		return this.baseDaoSupport.queryForPage("select * from tags", pageNo,
				pageSize);
	}
	...
	public void saveRels(Integer relid, Integer[] tagids) {
		// 清空原有引用
		String sql = "delete from " + this.getTableName("tag_rel")
				+ " where rel_id=?";
		this.daoSupport.execute(sql, relid);
		if (tagids != null) {

			// 重新对照新的引用
			sql = "insert into " + this.getTableName("tag_rel")
					+ "(tag_id,rel_id)values(?,?)";
	...
	public List<Integer> list(Integer relid) {
		String sql = "select tag_id from " + this.getTableName("tag_rel")
				+ " where rel_id=" + relid;
		List<Integer> tagids = this.jdbcTemplate.queryForList(sql,
				Integer.class);
		return tagids;
	}
