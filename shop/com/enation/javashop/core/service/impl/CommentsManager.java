...
public class CommentsManager extends BaseSupport<Comments> implements
	...
	public void deleteComments(String ids) {
		if (ids == null || ids.equals(""))
			return;
		String sql = "update comments set disabled='true' where comment_id in ("
				+ ids + ")";
	...
	public CommentDTO getComments(Integer commentId) {
		...
		List<Comments> list = baseDaoSupport
				.queryForList(
	...
	public void revert(String ids) {
		if (ids == null || ids.equals(""))
			return;
		String sql = "update comments set disabled='false' where comment_id in ("
				+ ids + ")";
		this.baseDaoSupport.execute(sql);
	}
	...
	public Page pageComments_Display(int pageNo, int pageSize,
		...
		String sql_id_list = "select * from "
				+ this.getTableName("comments")
				+ " where for_comment_id in (select comment_id from "
				+ this.getTableName("comments")
				+ " where for_comment_id is null and display='true' and object_id = ? and object_type=? ) and display='true' order by time desc ";
		List<Comments> listReply = this.daoSupport.queryForList(sql_id_list,
				Comments.class, object_id, object_type);
		...
		for (Map comments : list) {
			...
			for (Comments reply : listReply) {
				if (reply.getFor_comment_id().equals(
						Integer.valueOf(comments.get("comment_id").toString()))) {
					child.add(reply);
				}
			}
	 ...
	 * @return
	 */
	public Page listAll(int pageNo, int pageSize, Integer object_id,
			String commenttype) {
		...
		String sql_id_list = "select * from "
				...
				+ " where for_comment_id in (select comment_id from "
				+ this.getTableName("comments")
				+ " where for_comment_id is null and display='true' and object_id = ? and commenttype=? ) and display='true' order by time desc ";
		List<Comments> listReply = this.daoSupport.queryForList(sql_id_list,
				Comments.class, object_id, commenttype);
		List<Map> list = (List<Map>) (page.getResult());
		for (Map comments : list) {
	...
	public Page pageComments(int pageNo, int pageSize, String object_type) {
		...
		String sql = "select c.*, case c.commenttype when 'goods' then g.name when 'article' then a.title end name from "
				+ this.getTableName("comments")
				+ " c left join "
				+ this.getTableName("goods")
				+ " g on g.goods_id = c.object_id left join "
				+ this.getTableName("article")
				+ " a on a.id = c.object_id where  for_comment_id is null and c.disabled='false' and object_type=? order by time desc";
		...
		String sql_id_list = "select * from "
				...
				+ " where for_comment_id in (select comment_id from "
				+ this.getTableName("comments")
				+ " where for_comment_id is null and display='true' and object_type=? ) order by time desc ";
		List<Comments> listReply = this.daoSupport.queryForList(sql_id_list,
				Comments.class, object_type);
		List<Map> list = (List<Map>) (page.getResult());
		for (Map comments : list) {
			List<Comments> child = new ArrayList<Comments>();
			for (Comments reply : listReply) {
	...
	public Page pageComments_Display(int pageNo, int pageSize,
		...
		Page page = this.baseDaoSupport.queryForPage(sql, pageNo, pageSize,
				object_id, object_type, commentType);

		String sql_id_list = "select * from "
				+ this.getTableName("comments")
				+ " where for_comment_id in (select comment_id from "
				+ this.getTableName("comments")
				+ " where for_comment_id is null and display='true' and object_id = ? and object_type=? and commenttype = ?) and display='true' order by time desc ";
		List<Comments> listReply = this.daoSupport.queryForList(sql_id_list,
				Comments.class, object_id, object_type, commentType);
		List<Map> list = (List<Map>) (page.getResult());
		for (Map comments : list) {
			Long time = (Long) comments.get("time");
			comments.put("date", (new Date(time)));
	...
	public List listComments(int member_id, String object_type) {
		...
		List list = this.baseDaoSupport.queryForList(sql, Comments.class,
				member_id, object_type);
		return list;
	}
	...
	public Map coutObjectGrade(String commentType, Integer objectid) {
		...
		List<Map> gradeNumList = this.baseDaoSupport.queryForList(sql,
				commentType, objectid);
