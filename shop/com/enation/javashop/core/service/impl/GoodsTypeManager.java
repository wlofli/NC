 ...
 * 商品类型管理
 * 
 * @author kingapex 2010-1-10下午05:53:40
 */
public class GoodsTypeManager extends BaseSupport<GoodsType> implements
		IGoodsTypeManager {
	...
	public GoodsTypeDTO get(Integer type_id) {
		...
		GoodsTypeDTO type = (GoodsTypeDTO) baseDaoSupport.queryForObject(sql,
				GoodsTypeDTO.class, type_id);// (sql, new GoodsTypeMapper(),
												// type_id);该DTO继承商品类型对象，因为要转换类型的属性、参数、品牌值并返回
		if (type == null) {
			return null;
		}
		List<Attribute> propList = GoodsTypeUtil.converAttrFormString(type
				.getProps());
		ParamGroup[] paramGroups = GoodsTypeUtil.converFormString(type
				.getParams());
	...
	public List getBrandListByTypeId(int type_id) {
		String sql = "select b.name name ,b.brand_id brand_id,0 as num from "
				+ this.getTableName("type_brand")
				+ " tb inner join "
				+ this.getTableName("brand")
				+ " b  on    b.brand_id = tb.brand_id where tb.type_id=? and b.disabled=0";
	...
	private String getParamsByTypeId(int type_id) {
		...
		String sql = "select params from goods_type where disabled=0 and type_id="
				+ type_id;
	...
	/**
	 * 
	 * 将一个 Attribute 对象的List 生成Json字串,<br/>
	 * 此字串将会保存到数据库goods_type表的props字段 List 是根据客户端页面用户输入的属性信息生成的
	 * 
	 * @param propnames
	 * @param proptypes
	 * @param options
	...
	/**
	 * 将一个ParamGroup 对像的List 生成json字串 <br/>
	 * 此字串将会保存在数据库goods_type的params字段 或 js_goods中的params字段
	 * 不同的是：goods_type字段中的参数信息无参数值信息，而js_goods表中的params字段有参数值信息。 <br/>
	 * List 是根据是根据客户端页面用户输入的参数信息生成的
	 * 
	 * @param paramnum
	 * @param groupnames
	 * @param paramnames
	...
	public Integer save(GoodsType type) {
		...
		if (type.getType_id() != null) {
			...
			this.baseDaoSupport.update(typeTableName, type, "type_id="
					+ type_id);
			String sql = "delete from " + this.getTableName("type_brand")
					+ " where type_id = ?";
		...
		if (brand_id != null) {
			for (int i = 0; i < brand_id.length; i++) {
				...
				String sql = "insert into  " + this.getTableName("type_brand")
						+ "(type_id,brand_id) values(?,?)";
	...
	public void clean(Integer[] type_ids) {
		...
		sql = "delete from " + this.getTableName("type_brand")
				+ " where type_id in(" + ids + ")";
	...
	public void revert(Integer[] type_ids) {
		...
		String sql = "update  goods_type set disabled=0  where type_id in ("
				+ ids + ")";
		this.baseDaoSupport.execute(sql);
	}
