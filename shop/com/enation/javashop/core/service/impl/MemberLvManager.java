 ...
 * 会员级别管理
 * 
 * @author kingapex 2010-4-29下午11:04:43
 */
public class MemberLvManager extends BaseSupport<MemberLv> implements
		IMemberLvManager {
	...
	public Integer getDefaultLv() {
		...
		List<MemberLv> lvList = this.baseDaoSupport.queryForList(sql,
				MemberLv.class);
	...
	public MemberLv get(Integer lvId) {
		if (lvId != null && lvId != 0) {
			...
			MemberLv lv = this.baseDaoSupport.queryForObject(sql,
					MemberLv.class, lvId);
			return lv;
		} else {
			return null;
	...
	public List<MemberLv> list(String ids) {
		...
		String sql = "select * from member_lv where  lv_id in(" + ids
				+ ") order by lv_id";
		List lvlist = this.baseDaoSupport.queryForList(sql, MemberLv.class);
		return lvlist;
	...
	public MemberLv getByPoint(int point) {
		...
		List<MemberLv> list = this.baseDaoSupport.queryForList(sql,
				MemberLv.class, point);
