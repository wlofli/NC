...
public class OrderReportManager extends BaseSupport implements
	...
	public PaymentLog getPayment(Integer paymentId) {
		String sql = "select l.*, m.uname as member_name, o.sn from "
				+ this.getTableName("payment_logs") + " l left join "
				+ this.getTableName("member")
				+ " m on m.member_id = l.member_id left join "
				+ this.getTableName("order")
				+ " o on o.order_id = l.order_id where l.payment_id = ?";
	...
	public List<DeliveryItem> listDeliveryItem(Integer deliveryId) {
		...
		return this.baseDaoSupport.queryForList(sql, DeliveryItem.class,
				deliveryId);
	}
	...
	private Page list(int pageNo, int pageSize, String order, int type) {
		...
		String sql = "select l.*, m.uname as member_name, o.sn from "
				...
				+ " m on m.member_id = l.member_id left join "
				+ this.getTableName("order")
				+ " o on o.order_id = l.order_id where l.type = " + type;
		sql += " order by " + order;
	...
	public List listDelivery(Integer orderId, Integer type) {
		return this.baseDaoSupport.queryForList(
				"select * from delivery where order_id = ? and type = ?",
				orderId, type);
	}

	public List listPayLogs(Integer orderId, Integer type) {
		return this.baseDaoSupport.queryForList(
				"select * from payment_logs where order_id = ? and type = ?",
				orderId, type);
	}
