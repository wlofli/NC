 ...
 * 
 */
public class MemberPriceManager extends BaseSupport<GoodsLvPrice> implements
		IMemberPriceManager {
	...
	public List<GoodsLvPrice> listPriceByGid(int goodsid) {
		...
		return this.baseDaoSupport.queryForList(sql, GoodsLvPrice.class,
				goodsid);
	}
	...
	public void save(List<GoodsLvPrice> goodsPriceList) {
		...
		if (goodsPriceList != null && goodsPriceList.size() > 0) {
			this.baseDaoSupport.execute(
					"delete from  goods_lv_price  where goodsid=?",
					goodsPriceList.get(0).getGoodsid());
