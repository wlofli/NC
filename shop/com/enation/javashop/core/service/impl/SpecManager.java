 ...
 * 规格管理
 * 
 * @author kingapex 2010-3-7上午11:19:20
 */
public class SpecManager extends BaseSupport<Specification> implements
		ISpecManager {
	...
	public void add(Specification spec, List<SpecValue> valueList) {
		...
		for (SpecValue value : valueList) {
			...
			if (path != null) {
				path = path.replaceAll(EopSetting.IMG_SERVER_DOMAIN
	...
	public boolean checkUsed(Integer[] idArray) {
		...
		String sql = "select count(0)  from  goods_spec where spec_id in (-1,"
				+ idStr + ")";
	...
	public void edit(Specification spec, List<SpecValue> valueList) {
		...
		this.baseDaoSupport.update("specification", spec,
				"spec_id=" + spec.getSpec_id());
		for (SpecValue value : valueList) {
