...
public class GoodsSearchManager2 extends BaseSupport implements
		IGoodsSearchManager2 {
	...
	public Map<String, Object> getSelector(String url) {
		...
		/**
		 * 如果按类别搜索,则查询此类别,并传递给过虑器 如果未按类别搜索,则传递null
		 */
		Cat cat = getCat(url);

		Map selectorMap = new HashMap<String, String>();
		List<IGoodsSearchFilter> filterList = goodsSearchPluginBundle
				.getPluginList();
		for (IGoodsSearchFilter filter : filterList) {

			String urlFragment = UrlUtils.getParamStringValue(url,
					filter.getFilterId());
			String exSelfurl = UrlUtils
					.getExParamUrl(url, filter.getFilterId()).replaceAll(
							".html", ""); // 获取排除此过滤器的url

			List<SearchSelector> selectorList = filter.createSelectorList(cat,
					exSelfurl, urlFragment);
			...
			if (filter instanceof IMultiSelector) {
				Map multiSelector = ((IMultiSelector) filter)
						.createMultiSelector(cat, exSelfurl, urlFragment);
	...
	public List list(int pageNo, int pageSize, String url) {
		...
		List goodslist = this.daoSupport.queryForList(sql.toString(), pageNo,
				pageSize, mapper);
	...
	private final void filterGoods(Map<String, Object> goods, ResultSet rs) {
		List<IGoodsDataFilter> filterList = goodsDataFilterBundle
				.getPluginList();
