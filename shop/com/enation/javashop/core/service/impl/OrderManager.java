...
public class OrderManager extends BaseSupport implements IOrderManager {
	...
	public void savePrice(double price, int orderid) {
		...
		this.baseDaoSupport.execute(
				"update order set order_amount=? where order_id=?", price,
				orderid);
		this.baseDaoSupport.execute(
	...
	public Order add(Order order, String sessionid) {
		...
		OrderPrice orderPrice = cartManager.countPrice(sessionid,
				order.getShipping_id(), "" + order.getRegionid(), isProtected);
		...
		if (itemList.isEmpty() && pgkList.isEmpty() && giftList.isEmpty())
	...
	public Page list(int pageNO, int pageSize, int disabled, String searchkey,
		...
		if (!StringUtil.isEmpty(searchkey) && !StringUtil.isEmpty(searchValue)) {
			// page = this.saasDaoSupport.queryForPage(sql.toString(), pageNO,
			// pageSize, searchValue);
			page = this.baseDaoSupport.queryForPage(sql.toString(), pageNO,
					pageSize, Order.class, disabled, searchValue);
		} else {
			page = this.baseDaoSupport.queryForPage(sql.toString(), pageNO,
	...
	public List<Map> listGoodsItems(Integer orderId) {
		...
		String sql = "select items.*, g.image_default as image, p.sn as sn from "
				+ this.getTableName("order_items") + " items ";
		sql += " left join " + this.getTableName("goods")
				+ " g on g.goods_id = items.goods_id ";
		sql += " left join " + this.getTableName("product")
				+ " p on p.product_id = items.spec_id ";
	 ...
	 * 
	 * //计算商品重量 Double weight = cartManager.countGoodsWeight(sessionid);
	 * //计算商品原始价格 Double originalPrice = cartManager.countGoodsTotal(sessionid);
	 * //计算捆绑商品的总价，并加入订单总价中 Double pgkTotal =
	 * this.cartManager.countPgkTotal(sessionid); //计算团购总价 Double groupBuyTotal
	 * = this.cartManager.countGroupBuyTotal(sessionid);
	 * 
	 * originalPrice = CurrencyUtil.add(originalPrice,pgkTotal); originalPrice =
	 * CurrencyUtil.add(originalPrice,groupBuyTotal);
	 * 
	 * Integer point = this.cartManager.countPoint(sessionid);
	 * 
	 * //计算原始配置送费用 Double[] priceArray =
	 * this.dlyTypeManager.countPrice(shippingid, weight, originalPrice,
	 * regionid, isProtected); Double dlyPrice = priceArray[0];//费送费用
	 * 
	 * Member member = UserServiceFactory.getUserService().getCurrentMember();
	 * Double orderTotal =null; if(member!=null){ Double coupPrice
	 * =cartManager.countGoodsDiscountTotal(sessionid); //应用了商品优惠规则后的商品价格
	 * 
	 * //对订单价格和积分执行优惠 DiscountPrice discountPrice =
	 * this.promotionManager.applyOrderPmt(coupPrice, dlyPrice,point,
	 * member.getLv_id());
	 * 
	 * coupPrice=discountPrice.getOrderPrice() ; //优惠会后订单金额 dlyPrice =
	 * discountPrice.getShipFee(); point = discountPrice.getPoint(); //
	 * orderTotal = coupPrice+ dlyPrice; //订单总金额 orderTotal =
	 * CurrencyUtil.add(coupPrice, dlyPrice); //订单总金额 // Double reducePrice =
	 * originalPrice - coupPrice; Double reducePrice =
	 * CurrencyUtil.sub(originalPrice , coupPrice);
	 *//** 设置订单价格对象 **/
	/*
	 * orderPrice.setGoodsPrice(coupPrice); //商品金额，优惠后的
	 * orderPrice.setDiscountPrice(reducePrice); //优惠的金额
	 * orderPrice.setShippingPrice(dlyPrice); orderPrice.setPoint(point);
	 * 
	 * }else{ // orderTotal = originalPrice+ dlyPrice;
	 * //如果是非会员购买，订单总金额=商品原始金额+配送用 orderTotal = CurrencyUtil.add(originalPrice,
	 * dlyPrice); orderPrice.setPoint(point);
	 * orderPrice.setShippingPrice(dlyPrice); }
	 * 
	 * 
	 * 
	 * //计算保价费用 if(isProtected){ Double protectPrice = priceArray[1]; //
	 * orderTotal+=protectPrice; orderTotal=CurrencyUtil.add(orderTotal,
	 * protectPrice);
	 *//** 设置保价费用 **/
	/*
	 ...
	 * orderPrice.setOrderPrice(orderTotal); orderPrice.setWeight(weight);
	 * 
	 * return orderPrice; }
	 */
	...
	public String export(Date start, Date end) {
		...
		InputStream in = FileUtil
				.getResourceAsStream("com/enation/javashop/core/service/impl/order.xls");
		...
		if ("2".equals(EopSetting.RUNMODE)) {
			...
			filename = "/user/" + site.getUserid() + "/" + site.getId()
					+ "/order";
		} else {
			filename = "/order";
		}
		...
		filename = filename + "/order"
				+ com.enation.framework.util.DateUtil.getDatelineLong()
				+ ".xls";
