...
public class BrandManager extends BaseSupport<Brand> implements IBrandManager {
	...
	public List<Brand> listByTypeId(Integer typeid) {
		...
		List list = this.daoSupport
				.queryForList(sql, new BrandMapper(), typeid);
		return list;
	}

	/**
	 * 读取品牌详细 会将本地文件存储的图片地址前缀替换为静态资源服务器地址。
	 * 
	 * @param brand_id
	 * @return
	 */
	...
	public void add(Brand brand) {
		if (brand.getFile() != null && brand.getFileFileName() != null) {
			brand.setLogo(UploadUtil.upload(brand.getFile(),
					brand.getFileFileName(), "brand"));
		}
	...
	public void update(Brand brand) {
		...
		this.baseDaoSupport.update("brand", brand,
	...
	public List groupByCat() {
		// 取得商品分类的第一级列表
		List<Map> listCat = this.baseDaoSupport
				.queryForList("select * from goods_cat where parent_id = 0 order by cat_order");
		for (Map map : listCat) {
			List list = this.baseDaoSupport.queryForList(
					"select type_id from goods_cat where cat_path like '"
							+ map.get("cat_path").toString() + "%'",
					new StringMapper());
			String types = StringUtil.listToString(list, ",");
			List listid = this.baseDaoSupport.queryForList(
					"select brand_id from type_brand where type_id in ("
							+ types + ")", new StringMapper());
			String ids = StringUtil.listToString(listid, ",");
			List<Brand> listBrand = this.baseDaoSupport.queryForList(
					"select * from brand where brand_id in (" + ids + ")",
					Brand.class);
