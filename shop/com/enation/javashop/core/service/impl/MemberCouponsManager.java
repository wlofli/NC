...
public class MemberCouponsManager extends BaseSupport implements
	...
	public Page pageMemberCoupons(int pageNo, int pageSize) {
		...
		for (Map map : list) {
			if (((String) map.get("cpns_status")).equals("1")) {
				if (this.isLevelAllowuse(
						((Integer) map.get("pmt_id")).intValue(),
						member.getLv_id())) {
	...
	private void generateCoupon(int cpns_id, int member_id) {
		Long curTime = (new Date()).getTime();
		String sql = "select * from " + this.getTableName("coupons")
				+ " as c left join " + this.getTableName("promotion")
		...
		if (list != null && list.size() > 0) {
			...
			String couponCode = this.makeCouponCode(
					((Long) map.get("cpns_gen_quantity")).intValue() + 1,
					(String) map.get("cpns_prefix"), member_id);
