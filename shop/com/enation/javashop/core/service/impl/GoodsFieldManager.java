 ...
 * 
 */
public class GoodsFieldManager extends BaseSupport<GoodsField> implements
		IGoodsFieldManager {
	...
	private String getFieldTypeSql(GoodsField field) {
		return field.getEnglish_name()
				+ " "
				+ this.goodsFieldPluginBundle.getFieldPlugin(field)
						.getDataType();
	}

	public void edit(GoodsField goodsField) {
		GoodsField oldDataField = this.get(goodsField.getField_id());
		this.baseDaoSupport.update("goods_field", goodsField, "field_id="
				+ goodsField.getField_id());
		if (!oldDataField.getEnglish_name()
				.equals(goodsField.getEnglish_name())) {// 变更了字段名
			StringBuffer sql = new StringBuffer();
			sql.append("alter table ");
			sql.append(this.getTableName("goods"));
	...
	public GoodsField get(Integer fieldid) {
		...
		return this.baseDaoSupport.queryForObject(sql, GoodsField.class,
				fieldid);
	}
	...
	public void delete(Integer fieldid) {
		...
		// 删除模型中的相应字段
		String sql = "alter table " + this.getTableName("goods") + " drop "
				+ goodsField.getEnglish_name();
	...
	public void delete(String typeids) {
		String sql = "select * from goods_field where type_id in(" + typeids
				+ ")";
		List<GoodsField> fieldList = this.baseDaoSupport.queryForList(sql,
				GoodsField.class);
		for (GoodsField field : fieldList) {
			sql = "alter table " + this.getTableName("goods") + " drop column "
