 ...
 * 
 */
public class ReturnsOrderManager extends BaseSupport<ReturnsOrder> implements
		IReturnsOrderManager {
	...
	public void add(ReturnsOrder returnsOrder, Integer[] specids) {
		...
		this.orderFlowManager.applyReturns(returnsOrder.getOrderid(),
				returnsOrder.getState(), specids);
	...
	public ReturnsOrder get(Integer id) {
		ReturnsOrder order = baseDaoSupport.queryForObject(
				"select * from returns_order where id=?", ReturnsOrder.class,
				id);
	...
	public Page listAll(int pageNo, int pageSize) {
		return this.baseDaoSupport.queryForPage(
				"select * from returns_order order by add_time desc", pageNo,
				pageSize, ReturnsOrder.class);
	}
	...
	public List<ReturnsOrder> listMemberOrder() {
		...
		return this.baseDaoSupport.queryForList(
				"select * from returns_order where memberid =? ",
				ReturnsOrder.class, member.getMember_id());
	}

	public void updateState(Integer id, int state) {
		this.baseDaoSupport.execute(
				"update returns_order set state=? where id=?", state, id);
	}
	 ...
	 * 
	 * @see
	 * com.enation.javashop.core.service.IReturnsOrderManager#refuse(java.lang
	...
	public void refuse(Integer id, String reson) {
		...
		this.baseDaoSupport.execute(
				"update returns_order set state=?,refuse_reson=? where id=?",
				2, reson, id);
	}
