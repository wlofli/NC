 ...
 * 购物车业务实现
 * 
 * @author kingapex 2010-3-23下午03:30:50 edited by lzf 2011-10-08
 */
...
public class CartManager extends BaseSupport implements ICartManager {
		...
		if (count > 0) {
			this.baseDaoSupport
					.execute(
							...
							cart.getNum(), cart.getProduct_id(),
							cart.getSession_id(), cart.getItemtype());
		} else {
	...
	public List listGoods(String sessionid) {
		...
		sql.append("select g.cat_id as catid,g.goods_id,g.image_default, c.name ,  p.specs  ,g.mktprice,g.point,p.product_id,c.price,c.cart_id as cart_id,c.num as num,c.itemtype,c.addon  from "
	...
	public List listPgk(String sessionid) {
		StringBuffer sql = new StringBuffer();
		sql.append("select g.sn,g.goods_id,g.image_default,c.name,g.mktprice,g.point,p.product_id,c.price,c.cart_id as cart_id,c.num as num,c.itemtype  from "
				+ this.getTableName("cart")
				+ " c,"
				+ this.getTableName("product")
				+ " p,"
	...
	public List listGift(String sessionid) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select f.fo_id as goods_id,f.list_thumb image_default,f.fo_name as name, f.price as mktprice, f.score as point,f.fo_id as product_id,f.score as price,f.limit_purchases as limitnum,c.cart_id as cart_id,c.num as num,c.itemtype  from "
				+ this.getTableName("cart")
				+ " c,"
	...
	public List listGroupBuy(String sessionid) {
		StringBuffer sql = new StringBuffer();
		sql.append("select g.goods_id,g.image_default, c.name , g.mktprice,g.point,gb.groupid as product_id,c.price,c.cart_id as cart_id,c.num as num,c.itemtype,c.addon  from "
				+ this.getTableName("cart")
				+ " c,"
				+ this.getTableName("goods") + " g ");
		sql.append("," + this.getTableName("group_buy") + " gb ");
		sql.append("where c.itemtype=3 and c.product_id = gb.groupid and gb.goodsid= g.goods_id   and c.session_id=?");
		List<CartItem> list = this.daoSupport.queryForList(sql.toString(),
				new CartItemMapper(), sessionid);
	...
	public Double countGoodsTotal(String sessionid) {
		...
		Double price = (Double) this.baseDaoSupport.queryForObject(
				sql.toString(), new DoubleMapper(), sessionid);
		return price;
	}
	...
	public Integer countPoint(String sessionid) {
		...
		if (member != null) {
			...
			StringBuffer sql = new StringBuffer();
			sql.append("select c.*, g.goods_id, g.point from "
					+ this.getTableName("cart")
					+ " c,"
	...
	public Double countGoodsWeight(String sessionid) {
		...
		Double weight = (Double) this.baseDaoSupport.queryForObject(
				sql.toString(), new DoubleMapper(), sessionid);
		return weight;
	}
	...
	public OrderPrice countPrice(String sessionid, Integer shippingid,
		...
		if (regionid != null && shippingid != null && isProtected != null) {
			// 计算原始配置送费用
			Double[] priceArray = this.dlyTypeManager.countPrice(shippingid,
					weight, originalPrice, regionid, isProtected);
			dlyPrice = priceArray[0];// 费送费用

			if (member != null) { // 计算会员优惠
				// 对订单价格和积分执行优惠
				DiscountPrice discountPrice = this.promotionManager
						.applyOrderPmt(coupPrice, dlyPrice, point,
								member.getLv_id());
		 ...
		 * --------------------------------------
		 * 
		 * //计算捆绑商品的总价，并加入订单总价中 Double pgkTotal = countPgkTotal(sessionid);
		 * //计算团购总价 Double groupBuyTotal = countGroupBuyTotal(sessionid);
		 * 
