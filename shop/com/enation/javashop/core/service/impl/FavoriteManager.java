...
public class FavoriteManager extends BaseSupport implements IFavoriteManager {
	...
	public Page list(int pageNo, int pageSize) {
		...
		String sql = "select g.*, f.favorite_id from "
				+ this.getTableName("favorite") + " f left join "
				+ this.getTableName("goods") + " g on g.goods_id = f.goods_id";
	...
	public List list() {
		...
		return this.baseDaoSupport.queryForList(
				"select * from favorite where member_id=?", Favorite.class,
				member.getMember_id());
	}
