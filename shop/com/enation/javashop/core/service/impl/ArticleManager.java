...
public class ArticleManager extends BaseSupport implements IArticleManager {
	...
	public Article get(Integer id) {
		...
		try {
			article = (Article) this.baseDaoSupport.queryForObject(sql,
					Article.class, id);
	...
	public List listByCatId(Integer cat_id) {
		String cat_path = (String) this.baseDaoSupport.queryForObject(
				"select cat_path from article_cat where cat_id = ?",
				new StringMapper(), cat_id);
		String sql = "select a.*,b.cat_path from "
				+ this.getTableName("article") + " a left join "
				+ this.getTableName("article_cat")
				+ " b on b.cat_id = a.cat_id where cat_path like '" + cat_path
	...
	public String getCatName(Integer cat_id) {
		...
		try {
			cat_name = (String) this.baseDaoSupport.queryForObject(sql,
					String.class, cat_id);
		} catch (Exception ex) {
			cat_name = null;
		}
	...
	public List topListByCatId(Integer catId, Integer topNum) {
		String sql = "select * from article where cat_id=? order by create_time limit 0,"
				+ topNum;
