 ...
 * 总体关系为： <li>
 * 未支付不可以退款，支付后才能退款<br>
 * 未支付可以支付，支付完成后可以再支付</li>
 * 
 * <li>
 * 未发货不可以退货，发货后可以退货<br>
 * 未发货可以发货，发货完成后不可以再发货</li>
 * 
 * <li>
 * 订单标记为完成状态后不可以进行其它操作<br>
 * 订单标记为作废状态后不可以进行其它操作</li>
 * 
 * @author kingapex 2010-4-8上午09:07:00
...
public interface IOrderFlowManager {
	 ...
	 * 为某订单付款<br/>
	 * 对订单状态有如下影响：<br>
	 * <li>如果全额付款订单状态为<b>已付款</b>，付款状态为已付款</li> <li>
	 * 如果部分付款订单状态为<b>已付款</b>，付款状态为部分已付款</li>
	 ...
	 *             下列情形之一抛出此异常: <li>paymentLog为null</li> <li>order_id(订单id)为null
	 *             </li> <li>money(付款金额)为null</li>
	 * @throws ObjectNotFoundException
	 *             如果要支付的订单不存在
	 * @throws IllegalStateException
	 *             如果订单支付状态为已支付
	 * @throws RuntimeException
	 *             当使用预存款支付，且用户预存款不够支付金额时
	 ...
	 * 为某订单退款<br>
	 * 对订单状态有如下影响：<br>
	 * <li>如果全额退款订单状态为<b>已退款</b>，付款状态为<b>已退款</b></li> <li>
	 * 如果部分退款订单状态为<b>已退款</b>，付款状态为<b>部分退付款</b></li>
	 * 如果订单为非匿名购买且支付方式(paymentLog.pay_method)为“预存款支付”则影响到会员预存款：<br>
	 ...
	 * 
	 * @param delivery
	 *            货运单对象</br> 如果不指定物流费用和保价费用，则其默认值为0
	 * @param itemList
	 *            本次发货的明细
	 * @throws ObjectNotFoundException
	 *             如果要发货的订单不存在
	 ...
	 * @throws IllegalArgumentException
	 *             下列情形之一抛出此异常:<br>
	 *             <li>delivery为null</li> <li>delivery 对象的 order_id(订单id)为null</li>
	 *             <li>itemList为null 或为空</li> <li>
	 *             发货明细列表中的DeliveryItem对象下列属情有一个为空则抛出异常 goods_id, product_id、num
	 *             </li>
	 * @see com.enation.javashop.service.support.OrderStatus
	 */
	 ...
	 *             如果订单支付状态为已退货
	 * @throws IllegalArgumentException
	 *             下列情形之一抛出此异常: <li>delivery为null</li> <li>delivery 对象的
	 *             order_id(订单id)为null</li> <li>itemList为null 或为空</li> <li>
	 *             发货明细列表中的DeliveryItem对象下列属情有一个为空则抛出异常 goods_id, product_id、num
	 *             </li>
	 * @see com.enation.javashop.service.support.OrderStatus
	 */
