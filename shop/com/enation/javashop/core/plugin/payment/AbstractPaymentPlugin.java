 ...
 * 支付插件基类<br/>
 * 
 * @author kingapex 2010-9-25下午02:55:10
 */
public abstract class AbstractPaymentPlugin extends AutoRegisterPlugin {
	...
	protected String getCallBackUrl(PayCfg payCfg) {
		...
		return "http://" + serverName + ":" + port + contextPath
				+ "/payok_callback_" + payCfg.getType() + ".html";
	}
	...
	protected String getReturnUrl(PayCfg payCfg) {
		...
		String contextPath = request.getContextPath();
		return "http://" + serverName + ":" + port + contextPath
				+ "/pay_return_" + payCfg.getType() + ".html";
	}
	 ...
	 * if(StringUtil.isEmpty(ordersn)){ //order为-1是预存款充值 if(price==null) return
	 ...
	 * advanceLogs.setImport_money(price); advanceLogs.setMember_advance(price);
	 * advanceLogs.setShop_advance(price); advanceLogs.setMoney(price);
	 * advanceLogs.setMessage(""); advanceLogs.setMemo("在线充值");
	 * advanceLogsManager.add(advanceLogs); return ; } Order order =
	 * orderManager.get(ordersn); PaymentLog paymentLog = new PaymentLog();
	 * Member member = UserServiceFactory.getUserService().getCurrentMember();
	 * if(member!=null){ paymentLog.setMember_name(member.getUname());
