...
public class OrderPluginBundle extends AutoRegisterPluginsBundle {
				...
				for (IPlugin plugin : plugins) {
					if (plugin instanceof IOrderCreateEvent) {
						if (loger.isDebugEnabled()) {
							loger.debug("调用插件 : " + plugin.getName()
									+ " onCreate 开始...");
						}
	 ...
	 */
	public void onItemShip(DeliveryItem deliveryItem,
	 ...
	 */
	/*
	 * public String getAllocationHtml(OrderItem item){ String html = null; try{
	 * if (plugins != null) { for (IPlugin plugin : plugins) { if (plugin
	 * instanceof IOrderAllocationItemEvent) { if (loger.isDebugEnabled()) {
	 * loger.debug("调用插件 : " + plugin.getName() + " getAllocationHtml 开始..."); }
	 * IOrderAllocationItemEvent event = (IOrderAllocationItemEvent) plugin; if(
	 * event.canBeExecute(item.getCat_id()) ){ html =
	 * ((IOrderAllocationItemEvent) plugin).getAllocationStoreHtml(item); } if
	 * (loger.isDebugEnabled()) { loger.debug("调用插件 : " + plugin.getName() +
	 * " getAllocationHtml 结束."); } } } }
	 * 
	 * return html; }catch(RuntimeException e){ if(this.loger.isErrorEnabled())
	 ...
	 */
	/*
	 * public String getAllocationViewHtml(OrderItem item){ String html = null;
	 * try{ if (plugins != null) { for (IPlugin plugin : plugins) { if (plugin
	 * instanceof IOrderAllocationItemEvent) { if (loger.isDebugEnabled()) {
	 * loger.debug("调用插件 : " + plugin.getName() +
	 * " getAllocationViewHtml 开始..."); } IOrderAllocationItemEvent event =
	 * (IOrderAllocationItemEvent) plugin; if(
	 * event.canBeExecute(item.getCat_id()) ){ html =
	 * ((IOrderAllocationItemEvent) plugin).getAllocationViewHtml(item); } if
	 * (loger.isDebugEnabled()) { loger.debug("调用插件 : " + plugin.getName() +
	 * " getAllocationViewHtml 结束."); } } } }
	 * 
	 * return html; }catch(RuntimeException e){ if(this.loger.isErrorEnabled())
	 * this.loger.error("调用订单插件[getAllocationViewHtml]事件错误", e); throw e; }
	 * 
	 * }
	 */
			...
			if (plugins != null) {
				for (IPlugin plugin : plugins) {
					if (plugin instanceof IOrderAllocationItemEvent) {
						if (loger.isDebugEnabled()) {
							loger.debug("调用插件 : " + plugin.getName()
									+ " filterAlloViewItem 开始...");
						}
						IOrderAllocationItemEvent event = (IOrderAllocationItemEvent) plugin;
						if (event.canBeExecute(catid)) {
							((IOrderAllocationItemEvent) plugin)
									.filterAlloViewItem(values, rs);
						}
						if (loger.isDebugEnabled()) {
							loger.debug("调用插件 : " + plugin.getName()
									+ " filterAlloViewItem 结束.");
						}
					}
				}
			}
