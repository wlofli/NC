...
public class GoodsAdjunctPlugin extends AbstractGoodsPlugin {
	...
	public String onFillGoodsEditInput(Map goods, HttpServletRequest request) {
		...
		for (Map map : listGoodsAdjunct) {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				AdjunctItem adjunctItem = (AdjunctItem) JSONObject.toBean(
						jsonObject, AdjunctItem.class);
	...
	private void save(Map goods) {
		...
		String[] adjunct_name = httpRequest
				.getParameterValues("adjunct.adjunct_name");
		String[] adjunct_minnum = httpRequest
				.getParameterValues("adjunct.min_num");
		String[] adjunct_maxnum = httpRequest
				.getParameterValues("adjunct.max_num");
		String[] adjunct_setprice = httpRequest.getParameterValues("pricetype");
		String[] adjunct_price = httpRequest
				.getParameterValues("adjunct.price");
		...
		if (adjunct_name != null) {
			for (int i = 0; i < adjunct_name.length; i++) {
				String[] productids = httpRequest
						.getParameterValues("productid_" + i);
				GoodsAdjunct adjunct = new GoodsAdjunct();

				adjunct.setGoods_id(goodsId);
				adjunct.setAdjunct_name(adjunct_name[i]);
				adjunct.setMin_num((adjunct_minnum[i] == null ? null : Integer
						.valueOf(adjunct_minnum[i])));
				adjunct.setMax_num((adjunct_maxnum[i] == null ? null : Integer
						.valueOf(adjunct_maxnum[i])));
				adjunct.setSet_price(adjunct_setprice[i]);
				adjunct.setPrice((adjunct_price[i] == null ? null : Double
						.valueOf(adjunct_price[i])));
	...
	private String getItemJson(String[] productids) {
		...
		for (Map pro : proList) {
			Integer productid = Integer.valueOf(pro.get("product_id")
					.toString());
