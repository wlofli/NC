...
public class RelatedArticlesGoodsPlugin extends AbstractGoodsPlugin {
	...
	public String onFillGoodsEditInput(Map goods, HttpServletRequest request) {
		...
		List articleList = this.baseDaoSupport.queryForList(sql,
				goods.get("goods_id"));
	...
	private void processData(Map goods) {
		...
		this.baseDaoSupport.execute(
				"delete from goods_articles where goodsid=?", goodsid);
		int i = 0;
		if (articleidAr != null) {
			for (String articleid : articleidAr) {
				...
				this.baseDaoSupport
						.execute(
								"insert into goods_articles(articleid,goodsid,title)values(?,?,?)",
								articleid, goodsid, title);
