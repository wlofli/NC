 ...
 * 
 */
public class GalleryAdminPlugin extends DefaultAlbumPlugin implements
		IOnSettingInputShow, IGoodsDeleteEvent {
	...
	protected void proessPhoto(String[] picnames, Map goods) {
		...
		if (picnames == null) {
			// goods.put("image_file",
			// EopSetting.IMG_SERVER_DOMAIN+"/images/no_picture.jpg");
			// goods.put("image_default",
			// EopSetting.IMG_SERVER_DOMAIN+"/images/no_picture.jpg");
			return;
		}
		...
		for (int i = 0; i < picnames.length; i++) {
			...
			try {
				...
				album_pic_width = Integer.valueOf(getSettingValue(
						"album_pic_width").toString());
			...
			try {
				...
				this.goodsAlbumManager.createThumb(filepath, thumbName,
						thumbnail_pic_width, thumbnail_pic_height);

				// 生成小图
				thumbName = getThumbPath(filepath, "_small");
				this.goodsAlbumManager.createThumb(filepath, thumbName,
						detail_pic_width, detail_pic_height);

				// 生成大图
				thumbName = getThumbPath(filepath, "_big");
				this.goodsAlbumManager.createThumb(filepath, thumbName,
						album_pic_width, album_pic_height);
		...
		imgsString = imgsString.replaceAll(EopSetting.IMG_SERVER_DOMAIN
		...
		if (image_default != null) {
			image_default = image_default.replaceAll(
	...
	public Map<String, String> beforeSettingSave(HttpServletRequest request) {
		...
		String thumbnail_pic_width = request
				.getParameter("photo.thumbnail_pic_width");
		String thumbnail_pic_height = request
				.getParameter("photo.thumbnail_pic_height");
		String detail_pic_width = request
				.getParameter("photo.detail_pic_width");
		String detail_pic_height = request
				.getParameter("photo.detail_pic_height");
		String album_pic_height = request
				.getParameter("photo.album_pic_height");
