 ...
 * 
 */
public class GoodsSpecPlugin extends AbstractGoodsPlugin implements
		IGoodsDeleteEvent {
	...
	private void processGoods(Map goods, HttpServletRequest request) {
		...
		if (StringUtil.isEmpty((String) goods.get("cost"))) {
			goods.put("cost", 0);
		}
		if (StringUtil.isEmpty((String) goods.get("price"))) {
			goods.put("price", 0);
		}
		if (StringUtil.isEmpty((String) goods.get("weight"))) {
			goods.put("weight", 0);
		}
		if (StringUtil.isEmpty((String) goods.get("store"))) {
			goods.put("store", 0);
		}
	}
	 ...
	 * 处理有规格商品的逻辑 每一行（即每一个货品product）,有如下hidden： specids(规格id数组，以,号隔开)如：1,2
	 * specvids(规格值数组，以,号隔开)如21,20 productids货品id数组，如果为新增，则为空
	 * 
	 * @param goods
	 * @param request
	 */
	private void processSpec(Map goods, HttpServletRequest request) {
		...
		if ("1".equals(haveSpec)) {
			...
			for (String sn : sns) {
				...
				Integer productId = StringUtil.isEmpty(productids[i]) ? null
						: Integer.valueOf(productids[i]);
				...
				for (String specid : specids) {
					...
					specvalue.setSpec_value_id(Integer.valueOf(specvids[j]
							.trim()));
				...
				String[] specvalues = httpRequest
						.getParameterValues("specvalue_" + i);
				...
				// 生成会员价list
				if (lvidStr != null && lvidStr.length > 0) { // lzf add line
																// 20110114
					List<GoodsLvPrice> goodsLvPrices = this
							.createGoodsLvPrices(lvPriceStr, lvidStr, goodsId);
			...
			if (lvidStr != null && lvidStr.length > 0) { // lzf add line
															// 20110114
				List<GoodsLvPrice> goodsLvPrices = this.createGoodsLvPrices(
						lvPriceStr, lvidStr, goodsId);
				product.setGoodsLvPrices(goodsLvPrices);
			} // lzf add line 20110114
	 ...
	 * 
	 * @param lvPriceStr
	 *            会员价数组，数组的值类型为字串，考虑到一般由request中获取。
	 * @param lvidStr
	 *            会员级别id数组，数组的值类型为字串，考虑到一般由request中获取。
	 * @param goodsid
	 *            当前商品id，没有填充productid,此id在productmanager中添加数据时动态获取到并填充
	 * @return 生成的List<GoodsLvPrice>
	 */
	private List<GoodsLvPrice> createGoodsLvPrices(String[] lvPriceStr,
			String[] lvidStr, int goodsid) {
