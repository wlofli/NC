 ...
 * 预存款支付
 * 
 * @author kingapex 2010-5-26下午02:29:09
 */
public class DepositPlugin extends AbstractPaymentPlugin implements
	...
	public String onPay(PayCfg payCfg, Order order) {
		...
		if (member == null) {
			...
			String url = "";
			try {
				url = URLEncoder.encode("widget?type=paywidget&orderid="
						+ order.getOrder_id() + "&paymentid=" + payCfg.getId(),
						"UTF-8");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}

			return "<script>location.href='member_login.html?forward=" + url
					+ "'</script>";
		}

		return "<script>location.href='" + this.getReturnUrl(payCfg)
				+ "&ordersn=" + order.getSn() + "';</script>正在支付...";
