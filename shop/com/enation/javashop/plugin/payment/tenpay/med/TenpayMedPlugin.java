 ...
 * 财付通(中介担保)支付插件
 * 
 * @author kingapex 2010-4-13下午03:48:25
 */
public class TenpayMedPlugin extends AbstractPaymentPlugin implements
		IPaymentEvent {
	...
	public String onCallBack() {
		...
		MediPayResponseHandler resHandler = new MediPayResponseHandler(request,
				response);
	...
	public String onPay(PayCfg payCfg, Order order) {
		...
		MediPayRequestHandler reqHandler = new MediPayRequestHandler(request,
		...
		reqHandler.setParameter("mch_price",
				formatPrice(order.getOrder_amount() * 100)); // 商品总价，单位为分
