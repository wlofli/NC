...
public class ShopApp extends App {
	...
	protected void cleanCache() {
		...
		// 清除商品分类缓存
		CacheFactory.getCache(GoodsCatCacheProxy.CACHE_KEY).remove(
				GoodsCatCacheProxy.CACHE_KEY + "_" + userid + "_" + siteid
						+ "_0");

		// 清除文章分类缓存
		CacheFactory.getCache(ArticleCatCacheProxy.cacheName).remove(
				ArticleCatCacheProxy.cacheName + "_" + userid + "_" + siteid);
	...
	public void sessionDestroyed(String sessionid, EopSite site) {
		...
		if (site != null) {
			if (this.logger.isDebugEnabled()) {
				this.logger.debug("site get from session is userid["
						+ site.getUserid() + "]-siteid[" + site.getId() + "]");
			}
