...
public class FindPasswordWidget extends AbstractMemberWidget {
	...
	private void find() {
		...
		try {
			...
			helper.setTo(email);
			helper.setText("<html><body>" + username
					+ "您的密码已经被重置为:<span style='color:red'><b>" + newPwd
					+ "</b></span></body></html>", true);
			mailSender.send(message);

			this.memberManager.updatePassword(member.getMember_id(), newPwd);
			this.showSuccess("您的密码已经重置更新，请登录" + email + "查收您的新密码。", "登录页面",
					"member_login.html");
		...
		// }
		//
		// ThreadContextHolder.getSessionContext().setAttribute("can_find_pwd",
		// "yes");
	...
	public void restorePwd() {
		Object flag = ThreadContextHolder.getSessionContext().getAttribute(
				"can_find_pwd");
		if ("yes".equals(flag)) {
			HttpServletRequest request = ThreadContextHolder.getHttpRequest();
			String memberid = request.getParameter("memberid");
			String password = request.getParameter("password");
			this.memberManager.updatePassword(Integer.valueOf(memberid),
					password);
