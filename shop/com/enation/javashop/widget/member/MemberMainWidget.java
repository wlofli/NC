 ...
 * 抽像的会员中心挂件基类
 * 
 * @author kingapex 2010-3-11下午10:24:06
 */
public class MemberMainWidget extends AbstractWidget {
	...
	public String process(Map<String, String> params) {
		String servletPath = ThreadContextHolder.getHttpRequest()
				.getServletPath();
		...
		// 拦截未登录进入会员中心
		if (!servletPath.equals("/member_login.html")
			...
			if (!StringUtil.isEmpty(loginpage)) {
				freeMarkerPaser.setPageFolder(this.getThemePath() + "/"
						+ params.get("folder"));
		...
		String url = RequestUtil.getRequestUrl(ThreadContextHolder
				.getHttpRequest());
		...
		/**
		 * ------------------------- 解析会员中心挂件自定义页面 since version2.2.4
		 * -------------------------
		 */
		...
		if ("yes".equals(ThreadContextHolder.getHttpRequest().getParameter(
				"ajax"))) {
			return subHtml;
		} else {

			if (!widget.getShowMenu())
				return subHtml;
