 ...
 * 购物车checkout挂件
 * 
 * @author kingapex 2010-3-25下午06:05:19
 */
	...
	protected void display(Map<String, String> params) {
		...
		if (action == null || action.equals("")) {
			...
			if (member != null) {
				...
				List pmtList = this.promotionManager.list(originalTotal,
						member.getLv_id());
		...
		if ("loginBuy".equals(action)) {
			if (!StringUtil.isEmpty(loginpage)) {
				...
				this.setPageFolder(contextPath + "/themes/"
						+ site.getThemepath());
				this.setPageName(loginpage);
			} else {
				this.setPageName("loginBuy");
		...
		if ("showDlyType".equals(action)) {
			...
			List<DlyType> dlyTypeList = this.dlyTypeManager.list(weight,
					orderPrice, request.getParameter("regionid"));
		...
		if ("showOrderTotal".equals(action)) {
			...
			boolean isProtected = "1".equals(request
					.getParameter("isProtected"));

			OrderPrice orderPrice = this.cartManager.countPrice(sessionid,
					typeId, regionId, isProtected);

			/*
			 * NumberFormat nf = NumberFormat.getInstance();
			 * nf.setMaximumFractionDigits(2); this.putData("reducePrice",
			 * nf.format( orderPrice.getDiscountPrice() )) ;//优惠金额:减价的金额
			 * this.putData("originalTotal", orderPrice.getOriginalPrice() );
			 * ////商品总价格:即原始价格 this.putData("dlyPrice",
			 * orderPrice.getShippingPrice());
			 * 
			 * if(isProtected){
		...
		if ("createOrder".equals(action)) {
			try {
				...
				//
				// PayCfg payCfg =
				// this.paymentManager.get(order.getPayment_id());
