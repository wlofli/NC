 ...
 * 购物车挂件
 * 
 * @author kingapex 2010-3-23下午03:18:58
 */
public class CartWidget extends AbstractMemberWidget {
			...
			if (member == null) {
				...
				try {
					this.promotionManager
							.useCoupon(code, member.getMember_id());
					Coupons coupons = (Coupons) ThreadContextHolder
							.getSessionContext().getAttribute("coupon");
					this.showSuccess("优惠卷[" + coupons.getCpns_name()
							+ "]已经成功应用于购物车中}", "购物车", "cart.html");
	...
	private void add() {
		...
		if (itemtype.equals("2")) {
			boolean r = this.addGiftToCart(Integer.valueOf(productid),
					Integer.valueOf(num), sessionid);
			if (!r)
				return;

		} else if (itemtype.equals("3")) {
			this.addGoupbuyToCart(Integer.valueOf(productid),
					Integer.valueOf(num), sessionid);

		} else {// 添加商品或捆绑促销
			this.addGoodsToCart(Integer.valueOf(productid),
					Integer.valueOf(num), sessionid, Integer.valueOf(itemtype));
		}

		ThreadContextHolder
				.getHttpRequest()
				.getSession()
				.setAttribute("site_key",
						EopContext.getContext().getCurrentSite());
	}
	 ...
	 * @param sessionid
	 */
	private void addGoodsToCart(Integer productid, Integer num,
			String sessionid, Integer itemtype) {
	...
	private void processAdj(Cart cart) {
		...
		if (has_adj != null && has_adj.length > 0) {
			...
			for (Map adjGroup : adjGroupList) {
				...
				String[] proidArray = request.getParameterValues("adj_pro_id_"
						+ adjid);

				String[] adj_nums = request.getParameterValues("adj_num_"
						+ adjid);
				...
				for (int i = 0; i < proidArray.length; i++) {
					...
					adjprice = CurrencyUtil.mul(Double.valueOf(adj_nums[i]),
							adjprice);
		...
		if (member == null) {
			try {
				this.showError(
						"您尚未登录不能兑换赠品",
						"点击此处到登录页面",
						"member_login.html?forward="
								+ URLEncoder.encode(
										"cart.html?action=add&productid="
												+ giftid + "&itemtype=2",
										"UTF-8"));
			...
			FreeOffer freeOffer = this.freeOfferManager.get(Integer
					.valueOf(giftid));
	 ...
	 * 添加团购至购物车
	 */
	private boolean addGoupbuyToCart(Integer groupid, Integer num,
			String sessionid) {
	...
	private void list() {
		...
		Coupons coupons = (Coupons) ThreadContextHolder.getSessionContext()
				.getAttribute("coupon");
	...
	public void getTotal() {
		String sessionid = request.getSession().getId();
		/*
		 * Integer point = this.cartManager.countPoint(sessionid); //商品原始总价
		 * Double originalTotal
		 * =cartManager.countGoodsTotal(request.getSession().getId());
		 * //计算捆绑商品的总价，并加入订单总价中 Double pgkTotal =
		 * this.cartManager.countPgkTotal(sessionid); //计算团购总价 Double
		 * groupBuyTotal = this.cartManager.countGroupBuyTotal(sessionid);
		 * 
		 * 
		 * //应用了商品优惠的优惠价格 Double actualTotal =
		 * cartManager.countGoodsDiscountTotal(sessionid);
		 * 
		 * Member member =
		 * UserServiceFactory.getUserService().getCurrentMember();
		 * if(member!=null){ List pmtList =promotionManager.list(actualTotal,
		 * member.getLv_id()); this.putData("pmtList", pmtList); //应用的订单优惠的优惠价
		 * actualTotal= this.promotionManager.applyOrderPmt(actualTotal, 0D,
		 * point,member.getLv_id()).getOrderPrice(); }
		 * 
		 * 
		 * 
		 ...
		 * 
		 * actualTotal = CurrencyUtil.add(actualTotal,pgkTotal); actualTotal =
		 * CurrencyUtil.add(actualTotal,groupBuyTotal);
		 * 
		 * this.putData("originalTotal", originalTotal);
		 * this.putData("actualTotal", actualTotal);
		 */

		OrderPrice orderprice = this.cartManager.countPrice(sessionid, null,
				null, null);
		Member member = UserServiceFactory.getUserService().getCurrentMember();
		if (member != null) {
			List pmtList = promotionManager.list(orderprice.getGoodsPrice(),
					member.getLv_id());
