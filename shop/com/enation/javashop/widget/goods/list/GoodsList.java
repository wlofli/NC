...
public class GoodsList extends AbstractWidget {
	...
	/**
	 * 返回url中包含的参数，以“-”为开始，“.”为结束 例如goodslist-1-2-3,2-4.html
	 * 
	 * @param index
	 */
	...
	protected void display(Map<String, String> params) {
		...
		Settings setting = (Settings) JSONObject.toBean(
				JSONObject.fromObject(settingJson), Settings.class);
		this.putData("setting", null);
		if (setting != null) {
			// 处理商品图片大小
			if (setting.getThumbnail_pic_width() == null) {
				String defaultWidth = this.settingService.getSetting("photo",
						"thumbnail_pic_width");
			...
			if (setting.getThumbnail_pic_height() == null) {
				String defaultHeight = this.settingService.getSetting("photo",
			...
			// 处理列宽
			setting.setColumnwidth(Integer.valueOf(99 / Integer.valueOf(setting
					.getColumnnum())));
			...
			if (distype == null || "default".equals(distype)) {
				...
				Term term = (Term) JSONObject.toBean(
						JSONObject.fromObject(termJson), Term.class);
				...
				if (term != null) {
					...
					List goodsList = goodsDataProvider.list(term,
							setting.getGoodsNum());
			...
			if (distype == null || "morediv".equals(distype)) {
				...
				Collection configList = JSONArray.toCollection(configArray,
						Tab.class);
				...
				while (tabIterator.hasNext()) {
					...
					List goodsList = this.goodsDataProvider.list(tab.getTerm(),
							setting.getGoodsNum());
	...
	private void putAttrText(List<GoodsView> goodsList) {
		...
		List<Attribute> attrList = goodsTypeManager
				.getAttrListByTypeId(tempgoods.getType_id());
