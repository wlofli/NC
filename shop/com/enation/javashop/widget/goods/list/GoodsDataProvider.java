...
public class GoodsDataProvider extends BaseSupport {
	...
	public List list(Term term, int num) {
		...
		if (!StringUtil.isEmpty(keyword)) {
			sql.append(" and (name like '%" + keyword + "%' or brief like '%"
					+ keyword + "%' or intro like '%" + keyword + "%'   )");
		}
		...
		if (!StringUtil.isEmpty(catids)) {// lzf modified 20101210
			...
			for (String cat_id : ids) {
				...
				List<String> catIdList = this.baseDaoSupport.queryForList(
						"select cat_id from goods_cat where cat_path like '"
								+ cat.getCat_path() + "%'", new StringMapper());
		...
		List list = this.baseDaoSupport.queryForList(sql.toString(), 1, num,
				new GoodsListMapper());
	 ...
	 * @param discount
	 */
	private void applyMemPrice(List<GoodsView> itemList,
		...
		for (GoodsView item : itemList) {
			...
			for (GoodsLvPrice lvPrice : memPriceList) {
				if (item.getHasSpec() == 0
						&& item.getGoods_id().intValue() == lvPrice
								.getGoodsid()) {
